...
async function getAiUserPermission(username) {
  try {
    ...
    return {
      ...
      ...((settings.userPermissions || {})[username] || {}),
      username,
    };
  }
}
...
const INTEGRATION_FIELD_MAP = {
  ...
  backfill: "backfill", connectedBy: "connected_by", connectedAt: "connected_at",
...
async function getIntegrationSecret(id) {
  const rows = await select(
    ...
    `select=id,api_key_encrypted,webhook_token_hash&id=eq.${encodeURIComponent(id)}`
  );
  if (!rows.length) return null;
  return {
    id: rows[0].id,
...
async function reportingFactsList(query = {}) {
  ...
  q += `&limit=${Math.max(1, Math.min(50000, Number(limit) || 20000))}`;
  const rows = await select("reporting_facts", q);
