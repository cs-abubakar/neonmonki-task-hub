...
const rowToUser = (r) => ({
  ...
  discordUserId: r.discord_user_id || "", // "" pre-migration (015)
...
async function createUser(user) {
  const r = await insert("users", {
    ...
    // Written only when set: "" matches the 012/015 column defaults, and omitting
    // them keeps user creation working on a database that predates the migration.
    ...(user.clientId ? { client_id: user.clientId } : {}),
    ...(user.discordUserId ? { discord_user_id: user.discordUserId } : {}),
  });
  return rowToUser(r);
}
...
async function updateUser(username, fields) {
  ...
  if ("discordUserId" in fields) row.discord_user_id = fields.discordUserId || "";
...
async function getIntegrationSecret(id) {
  const rows = await select(
    ...
    `select=id,api_key_encrypted,webhook_token_hash,oauth_client_secret_encrypted,oauth_access_token_encrypted,oauth_refresh_token_encrypted,oauth_pending,oauth_client_id,oauth_access_expires_at,auth_method,meta&id=eq.${encodeURIComponent(id)}`
  );
  if (!rows.length) return null;
  return {
    id: rows[0].id,
...
/** Rolling retention: delete platform_daily rows older than `beforeDay`. */
async function platformDailyPrune({ beforeDay } = {}) {
  if (!beforeDay) throw new Error("platformDailyPrune: beforeDay is required");
  const rows = await req("DELETE", "platform_daily", {
    query: `select=id&day=lt.${encodeURIComponent(beforeDay)}`,
    prefer: "return=representation",
  });
  return Array.isArray(rows) ? rows.length : 0;
}
...
const DEFAULT_CLIENT = { id: "neonmonki", name: "NEONMONKI", active: true, notes: "", discordChannelId: "", discordEnabled: true, createdAt: null };

const rowToClient = (r) => ({
  id: r.id, name: r.name || "", active: r.active !== false,
  notes: r.notes || "",
  // Discord board routing (015); absent columns read as defaults pre-migration.
  discordChannelId: r.discord_channel_id || "",
  discordEnabled: r.discord_enabled !== false,
  createdAt: r.created_at || null,
});
...
async function clientInsert({ id, name, active, notes, discordChannelId, discordEnabled }) {
  const r = await insert("clients", {
    id, name: name || "", active: active !== false, notes: notes || "",
    // Conditional like the user columns: omitted entirely on pre-015 schemas.
    ...(discordChannelId ? { discord_channel_id: discordChannelId } : {}),
    ...(discordEnabled === false ? { discord_enabled: false } : {}),
  });
...
async function clientUpdate(id, fields) {
  ...
  if ("discordChannelId" in fields) row.discord_channel_id = fields.discordChannelId || "";
  if ("discordEnabled" in fields) row.discord_enabled = fields.discordEnabled !== false;
...
const rowToDiscordLog = (r) => ({
  id: r.id, eventKey: r.event_key || "", kind: r.kind || "",
  username: r.username || "", discordUserId: r.discord_user_id || "",
  taskId: r.task_id || "", boardId: r.board_id || "", channelId: r.channel_id || "",
  status: r.status || "sent", error: r.error || "", createdAt: r.created_at || null,
});

/** Has this exact event already been delivered? (overdue-sweep dedupe) */
async function discordLogSent(eventKey) {
  const rows = await select(
    "discord_log",
    `select=id&event_key=eq.${encodeURIComponent(eventKey)}&status=eq.sent&limit=1`
  );
  return rows.length > 0;
}

/** Upsert keyed on event_key so a failed ping can be re-attempted once fixed. */
async function discordLogWrite(entry) {
  const rows = await req("POST", "discord_log?on_conflict=event_key", {
    body: {
      event_key: entry.eventKey || "", kind: entry.kind || "",
      username: entry.username || "", discord_user_id: entry.discordUserId || "",
      task_id: entry.taskId || "", board_id: entry.boardId || "",
      channel_id: entry.channelId || "", status: entry.status || "sent",
      error: entry.error || "",
    },
    prefer: "resolution=merge-duplicates,return=representation",
  });
  return rows && rows.length ? rowToDiscordLog(rows[0]) : null;
}

async function discordLogRecent(limit = 20) {
  const rows = await select("discord_log", `select=*&order=id.desc&limit=${Math.min(100, limit) || 20}`);
  return rows.map(rowToDiscordLog);
}
...
module.exports = {
  ...
  platformDailyUpsert, platformDailyQuery, platformDailyDelete, platformDailyCount, platformDailyPrune,
  reportsList, reportInsert, reportUpdate, reportDelete,
  clientsList, clientInsert, clientUpdate,
  discordLogSent, discordLogWrite, discordLogRecent,
