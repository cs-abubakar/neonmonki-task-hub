...
/**
 * The cross-source overview behind the Platform Reports dashboard header:
 * one aligned answer across every connected source. Hyros (attribution)
 * carries revenue/spend/leads/sales; platform connectors carry their own
 * daily rows. Missing sources are simply absent from the payload — the UI
 * shows them as connect placeholders instead of fake zeros.
 */
async function combinedReport(store, { from, to }) {
  const span = dayCount(from, to);
  const cmpTo = shiftDays(from, -1);
  const cmpFrom = shiftDays(cmpTo, -span);
  ...
  const out = {
    from, to, cmpFrom, cmpTo,
    kpis: {}, trend: [], mix: [], sources: {},
  };

  // ---- Hyros attribution (present only when Smart Reporting is connected)
  if (typeof store.reportingDailyQuery === "function") {
    try {
      const [cur, prev] = await Promise.all([
        store.reportingDailyQuery({ from, to, sourceSystem: "hyros" }),
        store.reportingDailyQuery({ from: cmpFrom, to: cmpTo, sourceSystem: "hyros" }),
      ]);
      if (cur.length) {
        out.sources.hyros = true;
        const sum = (rows, scope, field) => rows.filter((r) => r.scope === scope)
          .reduce((n, r) => n + (Number(r[field]) || 0), 0);
        const agg = (rows) => ({
          revenue: sum(rows, "channel", "revenue"),
          leads: sum(rows, "channel", "leads"),
          sales: sum(rows, "channel", "sales"),
          spend: sum(rows, "account", "spend"),
        });
        const c = agg(cur);
        const p = agg(prev);
        const put = (key, value, prevValue, money) => {
          out.kpis[key] = { value, delta: pctDelta(value, prevValue), money: !!money };
        };
        put("revenue", c.revenue, p.revenue, true);
        put("spend", c.spend, p.spend, true);
        put("leads", c.leads, p.leads);
        put("sales", c.sales, p.sales);
        const roas = c.spend > 0 ? c.revenue / c.spend : null;
        const prevRoas = p.spend > 0 ? p.revenue / p.spend : null;
        out.kpis.roas = { value: roas, delta: roas != null && prevRoas ? pctDelta(roas, prevRoas) : null, ratio: true };
        // channel mix by attributed revenue
        const byChannel = new Map();
        for (const r of cur) {
          if (r.scope !== "channel") continue;
          const ch = r.channel || "Other";
          byChannel.set(ch, (byChannel.get(ch) || 0) + (Number(r.revenue) || 0));
        }
        out.mix = [...byChannel.entries()].map(([channel, value]) => ({ channel, value }))
          .filter((x) => x.value > 0).sort((a, b) => b.value - a.value);
      }
    } catch { /* reporting_daily not migrated here */ }
  }

  // ---- platform_daily rows (per-source daily series)
  const dayRows = await store.platformDailyQuery({ from, to, sliceType: "date" });
  const dayRowsPrev = await store.platformDailyQuery({ from: cmpFrom, to: cmpTo, sliceType: "date" });
  const clarityRows = await store.platformDailyQuery({ platform: "clarity", from, to, sliceType: "overall" });
  const clarityRowsPrev = await store.platformDailyQuery({ platform: "clarity", from: cmpFrom, to: cmpTo, sliceType: "overall" });

  const metricSum = (rows, platform, metric) => rows
    .filter((r) => r.platform === platform && (metric ? r.metric === metric : r.metric === ""))
    .reduce((n, r) => n + (Number(r.value ?? r.clicks) || 0), 0);

  const gscClicks = (rows) => rows.filter((r) => r.platform === "gsc" && r.metric === "")
    .reduce((n, r) => n + (Number(r.clicks) || 0), 0);
  if (dayRows.some((r) => r.platform === "gsc")) {
    out.sources.gsc = true;
    out.kpis.organicClicks = { value: gscClicks(dayRows), delta: pctDelta(gscClicks(dayRows), gscClicks(dayRowsPrev)) };
  }
  if (dayRows.some((r) => r.platform === "ga4")) {
    out.sources.ga4 = true;
    out.kpis.sessions = { value: metricSum(dayRows, "ga4", "sessions"), delta: pctDelta(metricSum(dayRows, "ga4", "sessions"), metricSum(dayRowsPrev, "ga4", "sessions")) };
    out.kpis.conversions = { value: metricSum(dayRows, "ga4", "conversions"), delta: pctDelta(metricSum(dayRows, "ga4", "conversions"), metricSum(dayRowsPrev, "ga4", "conversions")) };
  }
  if (dayRows.some((r) => r.platform === "salesforce")) {
    out.sources.salesforce = true;
    out.kpis.crmLeads = { value: metricSum(dayRows, "salesforce", "leads"), delta: pctDelta(metricSum(dayRows, "salesforce", "leads"), metricSum(dayRowsPrev, "salesforce", "leads")) };
    const won = metricSum(dayRows, "salesforce", "wonValue");
    out.kpis.wonValue = { value: won, delta: pctDelta(won, metricSum(dayRowsPrev, "salesforce", "wonValue")), money: true };
  }
  if (clarityRows.length) {
    out.sources.clarity = true;
    const cur = metricSum(clarityRows, "clarity", "Traffic:totalSessionCount");
    const prev = metricSum(clarityRowsPrev, "clarity", "Traffic:totalSessionCount");
    out.kpis.behaviorSessions = { value: cur, delta: pctDelta(cur, prev) };
  }
  for (const adPlatform of ["google_ads", "meta_ads"]) {
    if (dayRows.some((r) => r.platform === adPlatform)) {
      out.sources[adPlatform] = true;
    }
  }

  // ---- aligned daily trend (per-source series; missing days stay absent)
  const byDay = new Map();
  const ensure = (day) => {
    if (!byDay.has(day)) byDay.set(day, { day });
    return byDay.get(day);
  };
  if (out.sources.hyros) {
    try {
      const rows = await store.reportingDailyQuery({ from, to, sourceSystem: "hyros" });
      for (const r of rows) {
        const t = ensure(r.day);
        if (r.scope === "channel") {
          t.revenue = (t.revenue || 0) + (Number(r.revenue) || 0);
          t.leads = (t.leads || 0) + (Number(r.leads) || 0);
          t.sales = (t.sales || 0) + (Number(r.sales) || 0);
        } else if (r.scope === "account") {
          t.spend = (t.spend || 0) + (Number(r.spend) || 0);
        }
      }
    } catch { /* optional */ }
  }
  for (const r of dayRows) {
    const t = ensure(r.day);
    if (r.platform === "gsc" && r.metric === "") t.organicClicks = (t.organicClicks || 0) + (Number(r.clicks) || 0);
    if (r.platform === "ga4" && r.metric === "sessions") t.sessions = (t.sessions || 0) + (Number(r.value) || 0);
    if (r.platform === "ga4" && r.metric === "conversions") t.conversions = (t.conversions || 0) + (Number(r.value) || 0);
    if (r.platform === "salesforce" && r.metric === "leads") t.crmLeads = (t.crmLeads || 0) + (Number(r.value) || 0);
  }
  for (const r of clarityRows) {
    if (r.metric !== "Traffic:totalSessionCount") continue;
    ensure(r.day).behaviorSessions = (ensure(r.day).behaviorSessions || 0) + (Number(r.value) || 0);
  }
  out.trend = [...byDay.values()].sort((a, b) => a.day.localeCompare(b.day));
  return out;
}

module.exports.combinedReport = combinedReport;
