 ...
 *   tracked per process
 * - Rate limiting: HTTP 429 is retried automatically — initialize up to 5
 *   attempts, tools/call up to 3 — honoring the Retry-After header (capped
 *   at 30s) or falling back to exponential backoff with jitter. Only an
 *   exhausted 429 throws, as a `rateLimited` error (never `reconnectRequired`
 *   — the auth is fine). initialize is single-flighted per token and token
 *   refresh per refresh-token, so parallel syncs never trigger duplicate
 *   handshakes or refresh-token rotation races.
 *
 * READ-ONLY GUARANTEE
 * -------------------
...
/* Exhausted HTTP 429. Auth is fine, so reconnectRequired is NEVER set here —
 * the caller surfaces this as a transient "retrying automatically" state. */
function rateLimitError(retryAfter) {
  const e = new Error("Hyros rate limit reached — retrying automatically");
  e.status = 429;
  e.rateLimited = true;
  e.retryAfter = typeof retryAfter === "number" && Number.isFinite(retryAfter) ? retryAfter : null;
  return e;
}

const sleep = (ms) => new Promise((res) => setTimeout(res, ms));

/* Retry-After header → seconds (integer form, or HTTP-date form); null when absent/unparseable. */
...
/* Delay before the next retry after failed attempt `attempt` (1-based).
 * A Retry-After hint wins (capped at 30s, +100ms so we never retry earlier
 * than asked); otherwise exponential backoff 1s/2s/4s/8s + up to 25% jitter. */
function retryDelayMs(attempt, retryAfter) {
  if (typeof retryAfter === "number" && Number.isFinite(retryAfter)) {
    return Math.min(retryAfter, 30) * 1000 + 100;
  }
  return 1000 * Math.pow(2, attempt - 1) * (1 + Math.random() * 0.25);
}
...
/* Hyros rotates the refresh token on every use, so two parallel refreshes with
 * the same token would invalidate each other — refresh is single-flighted per
 * refresh-token hash; concurrent callers share the one in-flight exchange.
 * Entries are cleared as soon as the exchange settles. */
const refreshInFlight = new Map(); // refreshTokenHash → Promise<tokens>
...
async function ensureAccessToken(cfg) {
  ...
  const refreshHash = crypto.createHash("sha256").update(String(cfg.refreshToken)).digest("hex").slice(0, 16);
  let promise = refreshInFlight.get(refreshHash);
  if (!promise) {
    promise = (async () => {
      ...
      if (typeof cfg.persistTokens === "function") await cfg.persistTokens(tokens); // persist the rotation exactly once
      return tokens;
    })();
    refreshInFlight.set(refreshHash, promise);
    const clear = () => { if (refreshInFlight.get(refreshHash) === promise) refreshInFlight.delete(refreshHash); };
    promise.then(clear, clear);
  }
  const tokens = await promise;
  cfg.accessToken = tokens.accessToken;
  cfg.accessExpiresAt = tokens.accessExpiresAt;
  if (tokens.refreshToken) cfg.refreshToken = tokens.refreshToken; // rotated
  return cfg.accessToken;
}
...
async function mcpPost(token, payload, sessionId) {
  ...
  try {
    ...
    return {
      ...
      retryAfter: parseRetryAfter(res.headers), // seconds from Retry-After, or null
    };
  } catch (e) {
...
/* Single-flight initialize: concurrent callers for the same token share ONE
 * in-flight handshake — Hyros rate-limits initialize, and parallel handshakes
 * would both burn that budget and race the per-process session cache.
 * Entries are cleared as soon as the attempt settles. */
const initInFlight = new Map(); // tokenHash → Promise<sessionId>
...
async function ensureSession(token) {
  ...
  let promise = initInFlight.get(tokenHash);
  if (!promise) {
    promise = initializeSession(token, tokenHash);
    initInFlight.set(tokenHash, promise);
    const clear = () => { if (initInFlight.get(tokenHash) === promise) initInFlight.delete(tokenHash); };
    promise.then(clear, clear);
  }
  return promise;
}
...
async function initializeSession(token, tokenHash) {
  let last429 = null;
  for (let attempt = 1; attempt <= INIT_MAX_ATTEMPTS; attempt++) {
    ...
    for (const version of PROTOCOL_VERSIONS) {
      ...
      if (r.status === 429) { rateLimited = true; last429 = r; break; } // back off, then retry the whole handshake
      throw sanitizedError("Hyros MCP initialize failed", r.status);
    }
    if (!rateLimited) throw sanitizedError("Hyros MCP protocol version negotiation failed", 400);
    if (attempt < INIT_MAX_ATTEMPTS) await sleep(retryDelayMs(attempt, last429.retryAfter));
  }
  throw rateLimitError(last429 ? last429.retryAfter : null);
}
...
async function callTool(cfg, name, args = {}, _retried = false) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (r.status === 429) {
      // transient backpressure — honor Retry-After, retry, and only escalate
      // to the caller (as a rateLimited error, never a reconnect) when exhausted
      if (attempt < MAX_ATTEMPTS) { await sleep(retryDelayMs(attempt, r.retryAfter)); continue; }
      throw rateLimitError(r.retryAfter);
    }
  ...
  throw sanitizedError("Hyros MCP tool call failed"); // unreachable — every loop path returns or throws
}
