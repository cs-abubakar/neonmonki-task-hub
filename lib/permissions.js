...
/**
 * Reporting access tier — "full" | "basic" | "none".
 *   full  — the Smart Reporting dashboard and every /api/reporting/* endpoint
 ...
 */
const REPORTING_TIERS = ["full", "basic", "none"];
...
function reportingAccess(user, permission) {
  ...
  if (REPORTING_TIERS.includes(explicit)) return explicit;
...
/**
 * Smart Reporting gate — the owner rule is unchanged: the super admin
 * account "abubakar" always has access. Beyond that, access comes from the
 * AI permission record (store.getAiUserPermission): `reporting: "full"`
 * grants it, `reporting: "basic" | "none"` denies it, and the legacy
 * `smartReporting: true` flag still grants it for backward compat.
 */
function canUseSmartReporting(user, permission) {
  if (!user || user.active === false) return false;
  if (user.role === "super_admin" && user.username === "abubakar") return true;
  if (!permission) return false;
  if (REPORTING_TIERS.includes(permission.reporting)) return permission.reporting === "full";
  return permission.smartReporting === true;
}
