 ...
 *   external — sees ONLY tasks they are a named owner of or that they
 *              created themselves. Department membership only makes them
 *              assignable; it never widens visibility. Shared/team/internal
 *              visibility never applies.
...
function canSeeTask(user, task) {
  ...
  if (user.role === "external") {
    return task.createdByUsername === user.username
      || task.requestedBy === user.name
      || (task.ownerUsernames || []).includes(user.username);
  }
  if (user.role === "client") {
