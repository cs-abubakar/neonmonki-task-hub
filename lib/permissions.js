...
function canAccessChannel(channel, user) {
  ...
  // External partners are never auto-included: explicit membership only,
  // even for autoAll channels like General.
  if (user.role === "external") return !!memberOf(channel, user.username);
 ...
 *
 * Role boundaries checked before the visibility rules:
 *   external — sees ONLY tasks they own, created, or that belong to their
 *              department(s); shared/team/internal visibility never applies
 *   client   — scoped to tasks of their own client (task.clientId, default
 *              "neonmonki" on both sides)
 */
function canSeeTask(user, task) {
  ...
  if (user.role === "external") {
    ...
    if ((task.visibility || "shared") === "private") return false;
    const userDepts = new Set(decodeDepartmentIds(user.departments || []));
    return decodeDepartmentIds(task.departmentIds || task.assignedDept, task.department).some((id) => userDepts.has(id));
  }
  if (user.role === "client") {
    const tc = task.clientId || "neonmonki";
    const uc = user.clientId || "neonmonki";
    if (tc !== uc) return false;
  }
