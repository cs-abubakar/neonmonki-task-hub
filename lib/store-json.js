...
function load() {
  if (!fs.existsSync(DATA_FILE)) {
    ...
    // Discord notification layer (015): delivery audit + overdue dedupe
    seed.discordLog = [];
  ...
  // Discord notification layer (015)
  db.discordLog = Array.isArray(db.discordLog) ? db.discordLog : [];
...
const INTEGRATION_FIELDS = [
  ...
  "oauthPending",
  "backfill", "meta", "connectedBy", "connectedAt", "disconnectedBy", "disconnectedAt",
];
...
async function platformDailyPrune({ beforeDay } = {}) {
  ...
  return mutate((db) => {
    if (!Array.isArray(db.platformDaily)) db.platformDaily = [];
    const before = db.platformDaily.length;
    db.platformDaily = db.platformDaily.filter((r) => r.day >= beforeDay);
    return before - db.platformDaily.length;
  });
}
...
const rowToClient = (r) => ({
  ...
  discordEnabled: r.discordEnabled !== false,
  createdAt: r.createdAt || null,
});
...
async function clientUpdate(id, fields) {
  return mutate((db) => {
    ...
    if ("discordChannelId" in fields) row.discordChannelId = fields.discordChannelId || "";
    if ("discordEnabled" in fields) row.discordEnabled = fields.discordEnabled !== false;
    return rowToClient(row);
  });
}
...
const rowToDiscordLog = (r) => ({
  id: r.id, eventKey: r.eventKey || "", kind: r.kind || "",
  username: r.username || "", discordUserId: r.discordUserId || "",
  taskId: r.taskId || "", boardId: r.boardId || "", channelId: r.channelId || "",
  status: r.status || "sent", error: r.error || "", createdAt: r.createdAt || null,
});

/** Has this exact event already been delivered? (overdue-sweep dedupe) */
async function discordLogSent(eventKey) {
  return load().discordLog.some((r) => r.eventKey === eventKey && r.status === "sent");
}

/** Upsert by eventKey so a failed overdue ping can be re-attempted once fixed. */
async function discordLogWrite(entry) {
  return mutate((db) => {
    const next = {
      eventKey: entry.eventKey || "", kind: entry.kind || "",
      username: entry.username || "", discordUserId: entry.discordUserId || "",
      taskId: entry.taskId || "", boardId: entry.boardId || "",
      channelId: entry.channelId || "", status: entry.status || "sent",
      error: entry.error || "", createdAt: new Date().toISOString(),
    };
    const existing = db.discordLog.find((r) => r.eventKey === next.eventKey);
    if (existing) {
      Object.assign(existing, next);
      return rowToDiscordLog(existing);
    }
    db.meta.notifSeq = (db.meta.notifSeq || 0) + 1;
    next.id = db.meta.notifSeq;
    db.discordLog.push(next);
    if (db.discordLog.length > 5000) db.discordLog = db.discordLog.slice(-5000);
    return rowToDiscordLog(next);
  });
}

async function discordLogRecent(limit = 20) {
  return load().discordLog.slice(-limit).reverse().map(rowToDiscordLog);
}
