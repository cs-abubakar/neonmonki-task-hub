...
function encodeTaskSource(meta) {
  return TASK_SOURCE_PREFIX + JSON.stringify({
    ...
    createdByType: ["client", "external"].includes(meta.createdByType) ? meta.createdByType : "team",
  });
}
...
function enrichTask(task, users, departments) {
  ...
  return {
    ...
    createdByType: task.createdByType || sourceMeta.createdByType ||
      (creator && creator.role === "client" ? "client" : creator && creator.role === "external" ? "external" : "team"),
  };
}
