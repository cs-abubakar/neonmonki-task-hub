...
const ALLOWED_KIMI_BASE_URLS = [GLOBAL_KIMI_BASE_URL, CHINA_KIMI_BASE_URL];
...
async function probeConnection(provider, baseUrl, signal) {
  ...
  if (!modelsRes.ok) {
    return {
      ...
      error: `HTTP ${modelsRes.status} — ${modelsRes.status === 401
        ...
        : ((models.error && models.error.message) || modelsRes.statusText || "provider error")}.`,
    };
  }
...
/** Control-center connectivity check with safe China/global endpoint detection. */
async function testConnection(store) {
  ...
  try {
    const first = await probeConnection(provider, provider.baseUrl, ctrl.signal);
    if (first.ok) return { ...first, autoDetected: false };
    if (first.status === 401 && ALLOWED_KIMI_BASE_URLS.includes(provider.baseUrl)) {
      const alternate = provider.baseUrl === CHINA_KIMI_BASE_URL
        ? GLOBAL_KIMI_BASE_URL
        : CHINA_KIMI_BASE_URL;
      const second = await probeConnection(provider, alternate, ctrl.signal);
      if (second.ok) return { ...second, autoDetected: true };
      return {
        ok: false,
        error: `The key was rejected by both Kimi China and Global endpoints. ` +
          `Create an API key in the Kimi API platform (a Kimi app membership alone is not an API key).`,
      };
    }
    return first;
  } catch (e) {
