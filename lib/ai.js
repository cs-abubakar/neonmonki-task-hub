...
function allowedToolNames(names, role) {
  const base = Array.isArray(names)
    ? names.filter((name) => TOOL_NAMES.includes(name))
    : [...TOOL_NAMES];
  return role === "super_admin" ? base : base.filter((name) => !ADMIN_TOOL_NAMES.includes(name));
}
...
async function runAsk(store, user, question, { channelId, channelName, allowedTools, deep, lastVisit } = {}) {
  ...
  const allowed = new Set(allowedToolNames(allowedTools, user.role));
