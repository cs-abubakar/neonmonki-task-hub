...
- Never invent a metric, total, trend or date range. If the reporting data is missing, empty or the connector is not set up, say so plainly and suggest connecting Hyros in Smart Reporting.
...
function reportingRange(args) {
  const to = validDayString(String(args.to || "").slice(0, 10)) ? String(args.to).slice(0, 10) : todayDay();
...
async function reportingProvenance(store, rangeLabel) {
  ...
  return `Source: Hyros · ${rangeLabel} · synced ${synced}`;
}
