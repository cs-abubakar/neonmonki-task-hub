...
const { accessibleChannels, canSeeTask, visibleTasks, visibleLinks, canUseSmartReporting, reportingAccess } = require("./permissions");
...
${user.role === "client"
    ? "Address Adika directly as NEONMONKI's project lead; never call him a client or mention his internal access role. Answer in plain business language. Never expose internal team chatter, internal notes, or anything marked team-only. Adika may see only his permitted tasks, deliverables, decisions, files and channels.\nVoice: a calm, trusted account lead briefing a partner — plain, positive-but-honest business language, no agency jargon. Lead with outcomes and delivered value; numbers are welcome when they help him decide. Never be panic-inducing: whenever something is behind, blocked or underperforming, always pair it in the same breath with what is being done about it and the next step. Celebrate real progress, never oversell. When discussing marketing performance, stay calm and factual: state growth plainly, phrase declines neutrally (\"slightly softer than the previous period\") and always pair them with context or the next step — never alarm language. Never mention the name of the underlying data platform or vendor, or any internal sync or data-sourcing mechanics; the numbers are simply \"your synced marketing data\"."
...
${user.role === "client" ? `MARKETING PERFORMANCE DATA (your synced marketing data):
- When the question touches marketing performance — spend, revenue, ROAS, leads, sales, clicks, channels, campaigns, or "how are we doing" — answer ONLY from the marketing figures attached in this conversation (the <reporting-data> block). Those are the same numbers the Performance page shows.
- If no figures are attached for the period asked about, say plainly and calmly that those numbers are not available for that period yet — never estimate, and never blame missing numbers on anything technical.
- NEVER mention the underlying data platform or vendor by name, nor connectors, sync status, rate limits, snapshots or any other internal data-sourcing mechanics. The data is simply "your synced marketing data".
- Never invent a metric, total, trend or date range.
- Never claim causation between marketing actions and outcomes. Describe timing and magnitude, then label any explanation as a hypothesis.` : `SMART REPORTING DATA (synced marketing facts, first connector: Hyros):
...
/** Provenance footer for the client-safe basic digest — never names the source. */
async function basicReportingProvenance(store, rangeLabel) {
  let updated = "recently";
  try {
    if (typeof store.getIntegration === "function") {
      const integration = await store.getIntegration("hyros");
      updated = (integration && (integration.lastSyncAt || integration.lastWebhookAt)) || "recently";
    }
  } catch { /* integration record unavailable */ }
  return `Source: your synced marketing data · ${rangeLabel} · updated ${updated}`;
}

/**
 * Calm, client-safe digest for the BASIC reporting tier: the same numbers the
 * Performance page shows, without a single data-sourcing or diagnostic detail.
 * Returns "" when there is nothing worth attaching.
 */
async function basicReportingDigest(store, reporting, range) {
  const basic = await reporting.reportingBasic(range);
  const c = (basic && basic.current) || {};
  const channels = (basic && Array.isArray(basic.channels) ? basic.channels : []).slice(0, 3);
  const highlights = (basic && Array.isArray(basic.highlights) ? basic.highlights : []).slice(0, 3);
  const hasNumbers = ["revenue", "leads", "sales", "spend", "roas"].some((k) => c[k] != null);
  if (!hasNumbers && !channels.length && !highlights.length) return "";
  const eur = (v) => (v == null ? "—" : `€${Number(v).toLocaleString("en-US", { maximumFractionDigits: 2 })}`);
  const n2 = (v) => (v == null ? "—" : Number(v).toLocaleString("en-US"));
  const lines = [
    `Revenue: ${eur(c.revenue)}`,
    `Leads: ${n2(c.leads)}`,
    `Sales: ${n2(c.sales)}`,
    `Spend: ${eur(c.spend)}`,
    `ROAS: ${c.roas == null ? "—" : `${Math.round(c.roas * 100) / 100}x`}`,
    "All money in EUR.",
  ];
  if (channels.length) {
    lines.push("Top channels by revenue:");
    for (const ch of channels) lines.push(`- ${trunc((ch && ch.name) || "", 80)}: revenue ${eur(ch && ch.revenue)}`);
  }
  if (highlights.length) {
    lines.push("Highlights:");
    for (const h of highlights) {
      const text = trunc((h && h.text) || "", 180);
      if (text) lines.push(`- ${text}`);
    }
  }
  const prov = await basicReportingProvenance(store, `${range.from} → ${range.to}`);
  return `\n\n<reporting-data range="${range.from} → ${range.to}">\nMarketing performance figures for this range (the same numbers the Performance page shows):\n${lines.join("\n")}\n${prov}\nAnswer calmly and plainly from these figures. For anything outside this range or not listed here, say honestly that you cannot see it — never estimate or invent a number.\n</reporting-data>`;
}
  ...
  },
  {
    ...
    async run(store, user, args, cite) {
      ...
      const guides = {
        ...
        results: [
          "Marketing results sync automatically into the Performance page (Smart Reporting for advanced users), compared with the previous period; the Reports page archives weekly, monthly and annual report files.",
  ...
  },
  {
    ...
    async run(store, user, args, cite) {
      ...
      const text = [
        ...
        "",
        "Results this week:",
        ...(highlights.length ? highlights : ["- No results logged for this week yet — synced channels land in Smart Reporting automatically."]),
        "",
        "Shipped this week:",
...
async function runAsk(store, user, question, { channelId, channelName, allowedTools, deep, lastVisit, reportingContext } = {}) {
  ...
  // Tiered reporting access: "advanced"/"super" get the owner-grade digest,
  // "basic" the calm client-safe one, "none" no digest at all. The tool
  // allowlist below still keys off `smartReporting` exactly as before.
  const reportingTier = reportingAccess(user, reportingPermission);
  ...
  let reportingDigest = "";
  if (reportingTier !== "none" && isReportingQuestion(question)) {
    try {
      const reporting = reportingModule();
      if (reporting) {
        ...
        if (reportingTier === "advanced" || reportingTier === "super") {
          const digestFilters = {};
        ...
        } else if (typeof reporting.reportingBasic === "function") {
          // BASIC tier: the calm client-safe digest from the Performance page
          // numbers. No tools, no filters — the full-range basics only.
          reportingDigest = await basicReportingDigest(store, reporting, range);
        }
      }
