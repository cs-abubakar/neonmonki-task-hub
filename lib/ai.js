...
async function probeConnection(provider, baseUrl, signal, configuredModel) {
  if (baseUrl === KIMI_CODE_BASE_URL) {
    ...
    if (!response.ok) {
      return {
        ...
        error: `Kimi Code HTTP ${response.status} — ${providerError(data, response)}.`.slice(0, 260),
      };
    }
    return {
      ok: true,
      baseUrl,
      providerLabel: "Kimi Code Membership",
      model,
  ...
  return {
    ok: true,
    baseUrl,
    providerLabel: baseUrl === CHINA_KIMI_BASE_URL ? "Moonshot China API" : "Moonshot Global API",
    model,
    recommendedModel: model,
  ...
  try {
    ...
    return {
      ...
      error: codeAttempt && codeAttempt.status === 401
        ? "Kimi Code rejected the saved key. Re-enter the full key created in Kimi Code Console; keys are shown only once."
        : (attempts[0] && attempts[0].error) || "Kimi connection failed.",
    };
  } catch (e) {
...
function systemPrompt(user, scopeNote) {
  return `You are NEONMONKI AI, the work intelligence assistant inside the NEONMONKI Task Hub — a private collaboration system shared by NEONMONKI (premium B2B LED neon signage, Germany) and Advertidea (marketing agency team).
...
8. When asked what to reply, inspect the relevant channel context, then use draft_reply so the user receives a reusable reply card. When asked to create work, use draft_task with clear ownership, departments, due date and definition of done.`;
}
