...
const { accessibleChannels } = require("./permissions");

const KIMI_BASE_URL = () =>
  (process.env.KIMI_BASE_URL || "https://api.moonshot.ai/v1").replace(/\/$/, "");
...
async function chatCompletion({ messages, tools, model, maxTokens = 1400 }) {
  if (!KIMI_API_KEY()) {
...
async function testConnection() {
  if (!KIMI_API_KEY()) return { ok: false, error: "KIMI_API_KEY is not set (Vercel env vars)." };
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 20000);
  try {
    const [modelsRes, balRes] = await Promise.all([
      fetch(`${KIMI_BASE_URL()}/models`, { headers: { Authorization: `Bearer ${KIMI_API_KEY()}` }, signal: ctrl.signal }),
      fetch(`${KIMI_BASE_URL()}/users/me/balance`, { headers: { Authorization: `Bearer ${KIMI_API_KEY()}` }, signal: ctrl.signal }),
    ]);
...
const TOOLS = [
  {
    ...
    async run(store, user, args, cite) {
      let tasks = (await store.getState()).tasks;
  ...
  },
  {
    ...
    async run(store, user, args, cite) {
      const open = (await store.getState()).tasks.filter((t) => !["Completed", "Cancelled"].includes(t.status));
      const by = {};
      for (const t of open) {
  ...
  },
  {
    ...
    async run(store, user, args, cite) {
      const q = String(args.query).toLowerCase();
      const links = (await store.getState()).links.filter((l) =>
  ...
  },
  {
    ...
    description: "Propose a change to an existing task (status, priority, owner, due date, next action). Does NOT change anything — the human user approves and the system executes under their permissions.",
    params: {
      type: "object",
      properties: {
    ...
    async run(store, user, args) {
      ...
      const fields = {};
      for (const f of ["status", "priority", "owner", "dueDate", "nextAction"]) {
        if (args[f] !== undefined && args[f] !== "") fields[f] = trunc(args[f], 150);
      }
  ...
  },
  {
    ...
    async run(store, user, args) {
      return `ACTION_PROPOSAL ${JSON.stringify({
        ...
        workstream: trunc(args.workstream || "", 100),
      })}`;
    },
  },
];
 ...
 */
async function runAsk(store, user, question, { channelId, channelName } = {}) {
  const citations = [];
  const seen = new Set();
  ...
  let latencyMs = 0;
  let model = (await store.getAiSettings()).model;
  let answer = "";

  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    const resp = await chatCompletion({ messages, tools: toolSpecs, model });
...
async function runSummarize(store, user, kind, label, contextText, citations) {
  ...
  const resp = await chatCompletion({ messages, model: settings.model, maxTokens: 900 });
  return {
    answer: resp.content,
    citations,
    tools: [],
...
module.exports = { chatCompletion, testConnection, runAsk, runSummarize, stateSnapshot, systemPrompt };
