  ...
  switch (key) {
    ...
    case "GET /api/cron/hyros-sync": {
      ...
      if (platforms && typeof store.getIntegration === "function") {
        for (const platform of ["gsc", "clarity"]) {
          try {
    ...
    // Reading platform reports follows the reporting tiers (basic and up).
    // Connecting, syncing and disconnecting are Control Panel powers — the
    // super admin only, exactly like the Hyros integration mutations.
    ...
    case "GET /api/platforms": {
      ...
      const gscConfigured = !!platforms.gscOAuthEnv();
      return send(res, 200, {
        platforms: [
          { id: "gsc", name: "Google Search Console", ...(await platforms.platformStatus(store, "gsc")), oauthConfigured: gscConfigured },
          { id: "clarity", name: "Microsoft Clarity", ...(await platforms.platformStatus(store, "clarity")) },
        ],
      });
    }

    case "GET /api/platforms/gsc/report": {
      const me = await basicReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      ...
      const range = parseReportingRange(url);
      if (range.error) return send(res, 400, { error: range.error });
      return send(res, 200, await platforms.gscReport(store, range));
    }

    case "GET /api/platforms/clarity/report": {
      const me = await basicReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      return send(res, 200, await platforms.clarityReport(store));
    }
    ...
    case "GET /api/platforms/gsc/oauth/start": {
      ...
      if (!platforms || !platforms.gscOAuthEnv()) return fail("gsc-not-configured");
      const state = crypto.randomBytes(24).toString("base64url");
      const url = platforms.gscAuthUrl(requestOrigin(req), state);
      await store.putIntegration("gsc", {
        oauthPending: { stateHash: platforms.sha256Text(state), createdAt: new Date().toISOString() },
        updatedAt: new Date().toISOString(),
      });
      res.writeHead(302, { Location: url, "Cache-Control": "private, no-store" });
      return res.end();
    ...
    case "GET /api/platforms/gsc/oauth/callback": {
      ...
      // rides along, so the super-admin gate still applies.
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      const fail = (reason) => {
      ...
      try {
        ...
        const secrets = await store.getIntegrationSecret("gsc");
        const pending = (secrets && secrets.oauthPending) || {};
        const fresh = pending.createdAt && (Date.now() - Date.parse(pending.createdAt)) < 10 * 60 * 1000;
        if (!fresh || !pending.stateHash || platforms.sha256Text(state) !== pending.stateHash) {
          return fail("gsc-state");
        }
        const tokens = await platforms.gscExchangeCode(requestOrigin(req), code);
        ...
        await store.logActivity({
          ...
          text: `connected Google Search Console (${pick.siteUrl})`,
        });
        const result = await platforms.syncPlatform(store, "gsc");
        await store.logActivity({
          ts: new Date().toISOString(), taskId: null, by: me.name,
          text: result.ok
            ? `synced Google Search Console (${result.recordsIn} rows, ${result.window ? `${result.window.from} → ${result.window.to}` : "initial"})`
            : `GSC connected; first sync pending (${result.error || "retry from Platform Reports"})`,
        });
        res.writeHead(302, { Location: `/?platform=${result.ok ? "gsc-connected" : "gsc-sync-pending"}#/platformreports` });
        return res.end();
      } catch (e) {
        console.error("[gsc-oauth] callback failed:", e && e.status ? `HTTP ${e.status}` : "error");
    ...
    case "POST /api/platforms/clarity/connect": {
      ...
      if (token.length < 12 || token.length > 500 || /\s/.test(token)) {
        return send(res, 400, { error: "The Clarity API token must be 12–500 characters with no spaces." });
      }
    ...
    case "POST /api/platforms/:platform/sync": {
      ...
      if (!["gsc", "clarity"].includes(platform)) return send(res, 404, { error: "Unknown platform." });
    ...
    case "POST /api/platforms/:platform/disconnect": {
      ...
      await store.logActivity({
        ts: now, taskId: null, by: me.name,
        text: `disconnected ${platform === "gsc" ? "Google Search Console" : "Microsoft Clarity"} (${deleted} rows removed)`,
      });
...
const ROUTE_PATTERNS = [
  ...
  { re: /^\/api\/platforms\/(gsc|clarity)\/sync$/, key: "POST /api/platforms/:platform/sync", methods: ["POST"] },
  { re: /^\/api\/platforms\/(gsc|clarity)\/disconnect$/, key: "POST /api/platforms/:platform/disconnect", methods: ["POST"] },
