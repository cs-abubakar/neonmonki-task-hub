...
async function route(req, res, key, parts) {
  ...
  switch (key) {
    ...
    /* ------------------------------ report library ------------------------------ */
    // Curated report links (Drive docs, decks) shared with the whole
    // workspace: every signed-in role reads, team + super admin add, super
    // admin edits and deletes. Rows live in report_library (migration 011),
    // exposed through store.reportsList/reportInsert/reportUpdate/reportDelete.
    ...
    case "GET /api/reports": {
      ...
      return send(res, 200, { reports: await store.reportsList() });
    }
    ...
    case "POST /api/reports": {
      ...
      const report = await store.reportInsert({ ...parsed.fields, createdBy: me.username });
      await store.logActivity({
        ts: new Date().toISOString(), taskId: null, by: me.name,
