...
const { memberOf, canAccessChannel, accessibleChannels } = require("./permissions");
const ai = require("./ai");

const rec = (type, id, text) => `<record type="${type}" id="${id}">${text}</record>`;

/** AI gate: returns null when the call may proceed, else {status, error}. */
async function aiGate(store, me, feature) {
  const s = await store.getAiSettings();
  if (!s.enabled) return { status: 503, error: "AI is currently disabled by the super admin." };
  if (me.role === "client" && !s.allowClient) return { status: 403, error: "AI is not enabled for the client account." };
  if (s.features && s.features[feature] === false) return { status: 403, error: "This AI feature is disabled." };
  const used = await store.aiCallsToday(me.username);
  if (used >= s.dailyLimit) {
    return { status: 429, error: `Daily AI limit reached (${s.dailyLimit}). Ask the super admin to raise it.` };
  }
  return null;
}
...
async function route(req, res, key, parts) {
  ...
  switch (key) {
    ...
    case "GET /api/state": {
      ...
      return send(res, 200, { ...state, meta: { statuses: STATUSES, priorities: PRIORITIES } });
    }
    ...
    case "PATCH /api/tasks/:id": {
      ...
      const textFields = [
        ...
        "blocker", "deliverable", "deliverableLink", "nextAction", "dueDate", "update",
      ];
    ...
    case "POST /api/links": {
      ...
      const item = await insertWithFreshId(store, "links", "LNK", async (id) => {
        const it = {
          id,
          taskId: clean(body.taskId, 30),
          channelId: clean(body.channelId, 60),
    ...
    case "GET /api/chat/channels/:cid/messages": {
      ...
      const messages = await store.listMessages(channel.id, before ? Number(before) : null, 50);
      return send(res, 200, {
        messages,
        channel: {
    ...
    case "GET /api/ai/status": {
      ...
      return send(res, 200, {
        ...
        callsToday: await store.aiCallsToday(me.username),
        allowedForMe: isTeamRole(me.role) || s.allowClient,
      });
    }
    ...
    case "POST /api/ai/ask": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      const gate = await aiGate(store, me, "ask");
      if (gate) return send(res, gate.status, { error: gate.error });
      const body = await readBody(req);
      ...
      try {
        ...
        return send(res, 200, { answer: r.answer, citations: r.citations, drafts: r.drafts, proposals: r.proposals, model: r.model });
      } catch (e) {
        await store.aiLog({
    ...
    case "POST /api/ai/summarize/task/:id": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      const gate = await aiGate(store, me, "summaries");
      if (gate) return send(res, gate.status, { error: gate.error });
      ...
      const links = (await store.getState()).links.filter((l) => l.taskId === task.id);
    ...
    case "POST /api/ai/brief": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      const gate = await aiGate(store, me, "brief");
      if (gate) return send(res, gate.status, { error: gate.error });
      const snapshot = await ai.stateSnapshot(store, me);
      const { decisions, activity } = await store.getState();
    ...
    case "POST /api/ai/actions/execute": {
      ...
      if (type === "task_update") {
        const task = await store.getTask(clean(body.taskId, 30));
        if (!task) return send(res, 404, { error: "Task not found." });
        const src = body.fields && typeof body.fields === "object" ? body.fields : {};
        const fields = {};
        for (const f of ["owner", "dueDate", "nextAction"]) {
          if (f in src && src[f] !== "") fields[f] = clean(src[f], 300);
        }
        if ("priority" in src) fields.priority = oneOf(src.priority, PRIORITIES, task.priority);
        if ("status" in src) {
          const next = oneOf(src.status, STATUSES, null);
          if (!next) return send(res, 400, { error: "Unknown status." });
          if (next !== task.status) fields.status = next;
        }
        if (!Object.keys(fields).length) return send(res, 400, { error: "Nothing to change." });
        // same role rules as a manual edit: the AI's proposal gets NO extra privileges
        if (me.role === "client") {
          const onlyStatus = Object.keys(fields).every((f) => f === "status");
          if (!onlyStatus || !fields.status || !clientStatusAllowed(task, fields.status)) {
            return send(res, 403, { error: "That change is outside what the client account may do. The team applies it manually." });
          }
        }
        if (fields.status) {
          await store.pushTaskUpdate(task.id, {
            ts: now, by: `${me.name} (via AI proposal)`,
            text: `Status changed from “${task.status}” to “${fields.status}”.`,
            statusFrom: task.status, statusTo: fields.status,
          });
          await store.logActivity({ ts: now, taskId: task.id, by: me.name, text: `moved “${task.title}” to ${fields.status} (AI proposal)` });
        ...
        if (edited.length) {
          ...
          await store.logActivity({ ts: now, taskId: task.id, by: me.name, text: `edited “${task.title}” (${edited.join(", ")}) (AI proposal)` });
        }
        await store.updateTask(task.id, fields);
        await store.aiActionInsert({
          username: me.username, actionType: "task_update",
          payload: { taskId: task.id, title: task.title, fields },
          status: "executed", decidedBy: me.username, decidedAt: now,
          note: clean(body.reason, 200),
        });
        return send(res, 200, { ok: true, task: await store.getTask(task.id) });
      }
      ...
      if (type === "decision") {
        ...
        const item = await insertWithFreshId(store, "decisions", "DEC", async (id) => {
          const it = {
            ...
            topic: clean(body.topic, 200), rule,
            workstream: clean(body.workstream, 100),
            owner: clean(body.owner, 100),
          };
          await store.insertRow("decisions", it);
        ...
        await store.logActivity({ ts: now, taskId: null, by: me.name, text: `recorded decision “${item.topic || item.rule.slice(0, 40)}” (AI proposal)` });
        await store.aiActionInsert({
          username: me.username, actionType: "decision",
          payload: { id: item.id, topic: item.topic, rule: item.rule },
          status: "executed", decidedBy: me.username, decidedAt: now,
        });
    ...
    case "POST /api/ai/actions/decline": {
      ...
      const body = await readBody(req);
      await store.aiActionInsert({
        username: me.username, actionType: clean(body.type, 30) || "unknown",
        payload: { taskId: clean(body.taskId, 30), fields: body.fields || {}, topic: clean(body.topic, 200) },
        status: "rejected", decidedBy: me.username, decidedAt: new Date().toISOString(),
      });
      return send(res, 200, { ok: true });
    }
    ...
    case "GET /api/ai/admin": {
      ...
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      const s = await store.getAiSettings();
      const audit = await store.aiAuditList(50);
    ...
    case "PATCH /api/ai/admin": {
      ...
      // NOTE: there is deliberately no apiKey field — keys come from env vars only.
