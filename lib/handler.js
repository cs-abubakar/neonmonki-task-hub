...
/** Slug for a client id: lowercase alnum + dash, 2–40 chars. */
...
async function route(req, res, key, parts) {
  ...
  switch (key) {
    ...
    case "POST /api/tasks": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      // External partners never create tasks — they comment, change status and
      // share links on tasks already visible to them.
      if (me.role === "external") {
        return send(res, 403, { error: "External partners can't create tasks. Post an update on a task shared with you instead." });
      }
      const body = await readBody(req);
      ...
      } else if (body.clientId !== undefined && body.clientId !== null && clean(body.clientId, 60)) {
    ...
    case "POST /api/reporting/report": {
      ...
      try {
        ...
        await store.aiLog({ username: me.username, kind: "report", question: label, status: "ok" });
        return send(res, 200, {
    ...
    /* ------------------------------ clients ------------------------------ */
    // Every signed-in role reads the active client names (task and report
    // forms offer them as choices); the super admin also sees inactive rows
    // and is the only role that manages the registry.
    ...
    case "GET /api/clients": {
      ...
      if (me.role === "super_admin") {
        return send(res, 200, {
          clients: clients.map((c) => ({ id: c.id, name: c.name, active: c.active !== false })),
        });
      }
      return send(res, 200, {
        clients: clients.filter((c) => c.active !== false).map((c) => ({ id: c.id, name: c.name })),
      });
    }
    ...
    case "POST /api/clients": {
      ...
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.clientInsert !== "function") {
        return send(res, 503, { error: "The client registry is not available on this deployment yet." });
      }
      const body = await readBody(req);
      const name = clean(body.name, 80);
      if (name.length < 2) return send(res, 400, { error: "Client name must be 2–80 characters." });
      const id = clientIdFromName(name);
      if (!/^[a-z0-9-]{2,40}$/.test(id)) {
        return send(res, 400, { error: "Client name needs at least 2 letters or numbers." });
      }
      if ((await clientsRegistry(store)).some((c) => c.id === id)) {
        return send(res, 409, { error: "That client already exists." });
      }
      const client = await store.clientInsert({ id, name, active: true, notes: "" });
      await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: `added client ${name} (${id})` });
      return send(res, 201, { client });
    }

    case "PATCH /api/clients/:id": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.clientUpdate !== "function") {
        return send(res, 503, { error: "The client registry is not available on this deployment yet." });
      }
      const body = await readBody(req);
      const fields = {};
      ...
      if ("notes" in body) fields.notes = clean(body.notes, 500);
      if (!Object.keys(fields).length) return send(res, 400, { error: "No client fields to update." });
      const client = await store.clientUpdate(parts[0], fields);
      if (!client) return send(res, 404, { error: "Client not found." });
      await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: `updated client ${client.name} (${client.id})` });
...
const ROUTE_PATTERNS = [
  ...
  { re: /^\/api\/clients\/([\w-]+)$/, key: "PATCH /api/clients/:id", methods: ["PATCH"] },
