  ...
  switch (key) {
    ...
    case "GET /api/integrations/hyros/oauth/callback": {
      ...
      try {
        ...
        const test = hyros ? await hyros.testConnection(testCfg) : null;
        if (!test || !test.ok) return fail("oauth-test");
        ...
        await store.putIntegration(HYROS_INTEGRATION_ID, {
          status: "connected",
          authMethod: "oauth",
          accountName: clean(test.accountName || "", 120) || "NEONMONKI",
        ...
        const cfg = hyrosConfig(await store.getIntegrationSecret(HYROS_INTEGRATION_ID), store);
        const backfill = hyros ? await runHyrosBackfill(store, hyros, cfg, (existing && existing.historicalDays) || 90) : { recordsIn: 0, done: false };
        await store.logActivity({
          ts: new Date().toISOString(), taskId: null, by: me.name,
          text: `connected Hyros via OAuth (${backfill.recordsIn} records synced${backfill.done ? "" : ", history continues in background"})`,
        });
