...
function sendDownload(res, dataUrl, filename) {
  const match = String(dataUrl || "").match(/^data:([^;,]+);base64,([A-Za-z0-9+/=\r\n]+)$/);
  if (!match) return send(res, 404, { error: "File data is unavailable." });
  ...
  switch (key) {
    ...
    case "GET /api/users/basic": {
      ...
      const users = (await store.listUsers())
        ...
        .map((u) => ({ username: u.username, name: u.name, role: u.role, departments: decodeDepartmentIds(u.departments || []) }));
    ...
    case "PATCH /api/tasks/:id": {
      ...
      const textFields = [
        ...
        "deliverableLink", "nextAction", "dueDate", "update",
      ];
    ...
    case "DELETE /api/tasks/:id/comments/:commentId": {
      ...
      if (me.role !== "super_admin" && comment.authorUsername !== me.username) {
    ...
    case "POST /api/tasks/:id/subtasks": {
      ...
      const subtask = {
        ...
        clientVisible: task.visibility === "shared" && body.clientVisible === true,
    ...
    case "POST /api/tasks/:id/files": {
      ...
      if (!name || !match) return send(res, 400, { error: "Choose a valid file." });
      const size = Buffer.byteLength(match[2], "base64");
      if (size > 2_000_000) return send(res, 400, { error: "Files must be 2 MB or smaller." });
      ...
      const link = await insertWithFreshId(store, "links", "FILE", async (id) => {
        const record = {
          ...
          title: name, url: dataUrl, type: "task_file", owner: me.name, note: encodeFileMeta(meta),
        };
        await store.insertRow("links", record);
        return record;
      });
      await store.logActivity({ ts: now, taskId: task.id, by: me.name, text: `attached file “${name}”` });
      await notifyTaskFollowers(store, task, me, `${me.name} attached “${name}” to “${task.title}”`);
    ...
    case "PATCH /api/tasks/:id/files/:fileId": {
      ...
      if (!link || link.taskId !== task.id || link.type !== "task_file") return send(res, 404, { error: "File not found." });
      const meta = parseFileMeta(link.note);
      if (!meta) return send(res, 409, { error: "File review information is unavailable." });
      const body = await readBody(req);
      ...
      if (["approve", "reject", "deliver"].includes(action)) {
        if (!canReviewTask(me, task)) return send(res, 403, { error: "Only a task owner or super admin can review and deliver files." });
        if (action === "approve") {
        ...
        if (action === "deliver") {
          if (next.status !== "approved") return send(res, 409, { error: "Approve the file before delivering it." });
          const clientCanSee = context.users.some((u) => u.active && u.role === "client" && canSeeTask(u, task));
          if (!clientCanSee) return send(res, 409, { error: "Share this task with NEONMONKI before delivering its file." });
        ...
        if (me.role !== "client" || !next.deliveredToClient) return send(res, 403, { error: "This file has not been delivered to you." });
      ...
      await store.logActivity({ ts: now, taskId: task.id, by: me.name, text: `${action.replace(/_/g, " ")} file “${link.title}”` });
    ...
    case "GET /api/files/:fileId/download": {
      ...
      if (!link || link.type !== "task_file") return send(res, 404, { error: "File not found." });
      const context = await loadTaskContext(store, link.taskId);
      if (!context.task || !canSeeTask(me, context.task)) return send(res, 404, { error: "File not found." });
      const meta = parseFileMeta(link.note);
      if (!meta || (me.role === "client" && !meta.deliveredToClient && meta.uploadedBy !== me.username)) {
        return send(res, 404, { error: "File not found." });
      }
      return sendDownload(res, link.url, link.title);
    }
    ...
    case "DELETE /api/chat/messages/:messageId": {
      ...
      if (me.role !== "super_admin" && message.authorId !== me.username) {
    ...
    case "GET /api/notifications": {
      ...
      const items = (await store.listNotifications(me.username, 30)).map((item) => {
        if (!String(item.channelId || "").startsWith("comment:")) return item;
        return { ...item, commentId: item.channelId.slice("comment:".length), channelId: "" };
      });
    ...
    case "GET /api/admin/overview": {
      ...
      const [rawUsers, channels, workspace] = await Promise.all([store.listUsers(), store.listChannels(), store.getState()]);
      const users = rawUsers.map((u) => adminUserOf(u, workspace.departments));
    ...
    case "POST /api/admin/users": {
      ...
      return send(res, 201, { user: adminUserOf(user, workspace.departments) });
    }
    ...
    case "PATCH /api/admin/users/:username": {
      ...
      return send(res, 200, { user: adminUserOf(user) });
    }
    ...
    case "POST /api/ai/summarize/task/:id": {
      ...
      const links = visibleLinks(linkState.links, me, { tasks: linkState.tasks, channels: linkChannels })
        .filter((l) => l.taskId === task.id && l.type !== "task_file");
    ...
    case "GET /api/ai/admin": {
      ...
      return send(res, 200, {
        ...
        provider: {
          name: providerConfig.baseUrl === ai.KIMI_CODE_BASE_URL
    ...
    case "PATCH /api/ai/admin": {
      ...
      if ("baseUrl" in body) {
        ...
        if (!ai.ALLOWED_KIMI_BASE_URLS.includes(baseUrl)) {
          return send(res, 400, { error: "Choose Kimi Code Membership, Moonshot China, or Moonshot Global." });
        }
        const current = await store.getAiSettings();
...
function adminUserOf(user, departments) {
  return {
    username: user.username,
    role: user.role,
    name: user.name,
