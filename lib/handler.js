...
const ROLES = ["super_admin", "team", "client", "external"];
...
// Tasks may be name-assigned to the internal team AND to external partners
// (e.g. Mateen) — never to the client or the workspace bot. Assignment is
// what opens an external partner's visibility to that single task.
const isAssignableUser = (user) => user && user.username !== "advertidea"
  && (isTeamRole(user.role) || user.role === "external");
...
const SMART_REPORTING_ERROR = "Smart Reporting is not enabled for this account.";
const REPORT_GENERATOR_ERROR = "Report generation is not enabled for this account.";
...
// lib/report-writer.js composes the downloadable .docx reports. Loaded lazily
// like the reporting layer — deployments without it answer 503.
function reportWriterModule() {
  try { return require("./report-writer"); } catch { return null; }
}

// lib/platforms.js — Platform Reports connectors (GSC OAuth + Clarity token).
// Same lazy-load contract as the reporting layer.
function platformsModule() {
  try { return require("./platforms"); } catch { return null; }
}

// lib/discord.js — the Discord notification layer. Lazy like the other
// optional layers: if it is missing, the TMS simply skips Discord delivery.
function discordModule() {
  try { return require("./discord"); } catch { return null; }
}

/**
 * Smart Reporting gate for HTTP routes: 401 signed out, 403 unless the user's
 * reporting tier includes the advanced dashboard — tiers "advanced" and
 * "super" pass (see permissions.canUseSmartReporting; the owner account
 * "abubakar" always passes). Returns the authed user when allowed; otherwise
...
/**
 * Control Panel gate: the signed-in super admin only. Reporting *refresh* and
 * integration management live behind this — never behind a reporting tier, so
 * a client or team member with dashboard access can never trigger a sync.
 */
async function superAdminUser(req, res, store) {
  const me = await getAuth(req, store);
  if (!me) { send(res, 401, { error: "Not signed in." }); return null; }
  if (me.role !== "super_admin") { send(res, 403, { error: "Super admin only." }); return null; }
  return me;
}

/**
 * Report generator gate for HTTP routes: the "super" reporting tier only
 * (permissions.canGenerateReports). Same 401/403 pattern as the Smart
 * Reporting gate; returns the authed user when allowed.
 */
async function reportGeneratorUser(req, res, store) {
  const me = await getAuth(req, store);
  if (!me) { send(res, 401, { error: "Not signed in." }); return null; }
  const permission = typeof store.getAiUserPermission === "function"
    ? await store.getAiUserPermission(me.username)
    : null;
  if (!canGenerateReports(me, permission)) {
    send(res, 403, { error: REPORT_GENERATOR_ERROR });
    return null;
  }
  return me;
}
 ...
 * when the user's reporting tier is "none" — the "basic", "advanced" and
 * "super" tiers all read the same calm, client-safe payload. Returns the
 * authed user when allowed; otherwise the response is already sent.
 */
...
const REPORT_KINDS = ["weekly", "monthly", "special"];
const REPORT_MONTH_RE = /^\d{4}-\d{2}$/;

/** Report links may point at any http(s) URL (Google Drive, dashboards, ...). */
function reportLinkUrl(value) {
...
/**
 * Validate report-library fields. A full POST requires title, periodMonth and
 * 1–6 links; with `partial` (PATCH) only the keys the body carries are checked.
 * Returns { fields } or { error }.
 */
function reportFieldsFrom(body, { partial = false } = {}) {
  const fields = {};
  const has = (key) => body[key] !== undefined && body[key] !== null;

  if (!partial || has("title")) {
    const title = clean(body.title, 140);
    if (title.length < 2) return { error: "Report title must be 2–140 characters." };
    fields.title = title;
  }
  if (!partial || has("description")) {
    fields.description = clean(body.description, 500);
  }
  if (!partial || has("kind")) {
    const kind = clean(body.kind, 20) || "weekly";
    if (!REPORT_KINDS.includes(kind)) return { error: "Report kind must be weekly, monthly or special." };
    fields.kind = kind;
  }
  if (!partial || has("periodMonth")) {
    const periodMonth = clean(body.periodMonth, 7);
    if (!REPORT_MONTH_RE.test(periodMonth)) return { error: "Report month must be YYYY-MM." };
    fields.periodMonth = periodMonth;
  }
  if (!partial || has("links")) {
    const rawLinks = Array.isArray(body.links) ? body.links : [];
    if (rawLinks.length < 1 || rawLinks.length > 6) return { error: "A report needs 1–6 links." };
    const links = [];
    for (const entry of rawLinks) {
      const url = reportLinkUrl(entry && entry.url);
      if (!url) return { error: "Every report link needs a valid http(s) URL." };
      links.push({ label: clean(entry && entry.label, 80), url });
    }
    fields.links = links;
  }
  return { fields };
...
// The client registry scopes client-role users, tasks and report-library rows.
// Both stores expose clientsList/clientInsert/clientUpdate (migration 012) and
// read an empty collection as the built-in NEONMONKI default; the fallback
// here keeps deployments alive where that store layer is not merged yet.
const DEFAULT_CLIENTS = [{ id: "neonmonki", name: "NEONMONKI", active: true, notes: "" }];

async function clientsRegistry(store) {
  return typeof store.clientsList === "function" ? store.clientsList() : DEFAULT_CLIENTS;
}

/** Validate a clientId against the registry; returns { clientId } or { error }. */
async function clientIdChecked(store, value) {
  const clientId = clean(value, 60) || "neonmonki";
  const clients = await clientsRegistry(store);
...
async function notifyCommentRecipients(store, task, actor, comment, users, origin) {
  ...
  for (const username of recipients) {
    ...
    const kind = explicit.includes(username) || everyone ? "mention" : "comment";
    await store.notify({
      username,
      kind: kind === "mention" ? "mention" : "task_comment",
      text: `${actor.name} commented on “${task.title}”: ${comment.text.slice(0, 140)}`,
      taskId: task.id,
      channelId: `comment:${comment.id}`,
    });
    discordTargets.push({ username, kind });
  }
  // Discord mirrors the exact in-app recipient set (mention vs comment); the
  // module re-checks visibility, Discord IDs and per-board channels itself.
  const discord = discordModule();
  if (discord && discordTargets.length) {
    await discord.deliver(store, {
      task, actor, targets: discordTargets, excerpt: comment.text,
      origin: origin || requestOrigin({ headers: {} }), eventId: comment.id,
    });
  }
}
...
async function route(req, res, key, parts) {
  ...
  switch (key) {
    ...
    case "GET /api/users/basic": {
      ...
      if (me.role === "external") {
        // External partners get the minimum assignment directory: who can take
        // work, by name and role — no profiles, contacts, or client accounts.
        const users = (await store.listUsers())
          .filter((u) => u.active && u.username !== "advertidea" && u.role !== "client")
          .map((u) => ({ username: u.username, name: u.name, role: u.role, departments: [] }));
    ...
    case "POST /api/search/answer": {
      ...
      try {
        ...
        await store.aiLog({
          username: me.username, kind: "search", question, answer: result.answer || "",
    ...
    case "GET /api/state": {
      ...
      return send(res, 200, {
        ...
        activity,
        lastVisit,
        team: me.role === "client" || me.role === "external" ? [] : state.team,
    ...
    case "POST /api/tasks": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      // External partners (e.g. Mateen) may raise tasks and assign them to
      // internal people or departments — that never widens their visibility
      // (they still only ever see tasks they own or created).
      const body = await readBody(req);
      ...
      // External partners file internal work only — never client-shared, never
      // private. The team decides later whether NEONMONKI should see it.
      if (me.role === "external" && !["team", "department"].includes(visibility)) visibility = "team";
      if (visibility === "private" && !ownerUsernames.length) {
        return send(res, 400, { error: "A private task needs at least one named owner." });
      }
      // Tasks carry a client scope: client accounts always file under their own
      // client; the team may choose any registered client (default NEONMONKI).
      // External partners always file under the NEONMONKI workspace itself.
      let clientId = "neonmonki";
      if (me.role === "client") {
        clientId = me.clientId || "neonmonki";
      } else if (me.role !== "external" && body.clientId !== undefined && body.clientId !== null && clean(body.clientId, 60)) {
        const checked = await clientIdChecked(store, body.clientId);
      ...
      const task = await insertWithFreshId(store, "tasks", "NM-NEW", async (id) => {
        const t = {
          ...
          requestedBy: me.role === "client" || me.role === "external" ? me.name : clean(body.requestedBy, 100) || me.name,
          owner,
          supporting: clean(body.supporting, 150),
          priority: oneOf(body.priority, PRIORITIES, "Medium"),
          status: me.role === "client" || me.role === "external" ? "New Request" : oneOf(body.status, STATUSES, "Planned"),
          evidence: "Current",
          update: "",
          ...
          source: encodeTaskSource({
            ...
            createdByType: me.role === "client" ? "client" : me.role === "external" ? "external" : "team",
          }),
          visibility,
          privateFor,
          clientId,
          ...
          updates: [{
            ...
            text: me.role === "client"
              ...
              : me.role === "external"
                ? "Task raised by an external partner. Waiting for the team to accept."
      ...
      for (const user of users) {
        if (!user.active || user.username === me.username) continue;
        // The internal team hears about new work; an external partner hears
        // about it only when they are a named owner of this task.
        const externalOwner = user.role === "external" && ownerUsernames.includes(user.username);
        if (!isTeamRole(user.role) && !externalOwner) continue;
      ...
      // Discord: assignment pings go only to the named owners (department /
      // whole-team routing has no individual target in Discord).
      const discord = discordModule();
      if (discord && ownerUsernames.length) {
        await discord.deliver(store, {
          task: modeled, actor: me, origin: requestOrigin(req),
          eventId: `create-${task.id}`,
          targets: ownerUsernames.map((username) => ({ username, kind: "assigned" })),
        });
      }
    ...
    case "PATCH /api/tasks/:id": {
      ...
      // External partners move the status only — every field edit (incl.
      // assignment, priority, due date, department, visibility) stays team-side.
      if (me.role === "external" && wantsFields) {
        return send(res, 403, {
          error: "Task details are managed by the team. Post a comment if something needs to change.",
        });
      }
      // …and closing work is a team call: externals hand over for review.
      if (me.role === "external" && wantsStatus && ["Completed", "Cancelled"].includes(body.status)) {
        return send(res, 403, { error: "Only the team can complete or cancel tasks — move your work to Ready for Review instead." });
      }
      ...
      // Discord: owners newly added by this edit get an assignment ping
      // ("reassigned" when the task already had owners). Removed owners are
      // not notified — the in-app activity trail covers the change.
      const discord = discordModule();
      if (discord && ("ownerUsernames" in body || "assignmentMode" in body)) {
        const before = new Set(task.ownerUsernames || []);
        const added = patchedOwnerUsernames.filter((username) => !before.has(username) && username !== me.username);
        if (added.length) {
          const modeled = taskModel(updated, context.users, context.workspace.departments);
          await discord.deliver(store, {
            task: modeled, actor: me, origin: requestOrigin(req),
            eventId: `reassign-${task.id}-${now}`,
            reassigned: before.size > 0,
            targets: added.map((username) => ({ username, kind: "assigned" })),
          });
        }
      }
    ...
    case "POST /api/tasks/:id/accept": {
      ...
      const updated = await store.getTask(id);
      // Discord: accepting a request assigns the owners — ping them.
      const discordAccept = discordModule();
      if (discordAccept && ownerUsernames.length) {
        const modeled = taskModel(updated, context.users, context.workspace.departments);
        await discordAccept.deliver(store, {
          task: modeled, actor: me, origin: requestOrigin(req),
          eventId: `accept-${task.id}`,
          targets: ownerUsernames.filter((u) => u !== me.username).map((username) => ({ username, kind: "assigned" })),
        });
      }
    ...
    case "DELETE /api/tasks/:id": {
      ...
      const ownUnacceptedExternal = me.role === "external" && task.status === "New Request"
        && (task.createdByUsername === me.username || task.requestedBy === me.name);
      const allowed = me.role === "super_admin" || ownUnacceptedRequest || ownUnacceptedExternal ||
    ...
    case "POST /api/tasks/:id/updates": {
      ...
      if (text) {
        ...
        // Discord: a progress update pings the task's named owners.
        const discord = discordModule();
        const ownerTargets = (task.ownerUsernames || [])
          .filter((username) => username !== me.username)
          .map((username) => ({ username, kind: "comment" }));
        if (discord && ownerTargets.length) {
          await discord.deliver(store, {
            task, actor: me, targets: ownerTargets, excerpt: text,
            origin: requestOrigin(req), eventId: `update-${task.id}-${now}`,
          });
        }
      }
      if (wantsStatus) {
    ...
    case "POST /api/tasks/:id/comments": {
      ...
      await notifyCommentRecipients(store, task, me, comment, context.users, requestOrigin(req));
    ...
    case "POST /api/tasks/:id/subtasks": {
      ...
      const clientCanPlan = me.role === "client" && task.status === "New Request"
        && (task.createdByUsername === me.username || task.requestedBy === me.name);
      const externalCanPlan = me.role === "external" && task.status === "New Request"
        && (task.createdByUsername === me.username || task.requestedBy === me.name);
      if (!isTeamRole(me.role) && !clientCanPlan && !externalCanPlan) {
    ...
    case "POST /api/decisions": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role === "external") return send(res, 403, { error: "Decisions are recorded by the team." });
      const body = await readBody(req);
    ...
    // data-quality diagnostics. Open to every reporting tier above "none"
    // ("basic", "advanced", "super"); 403 only when reportingAccess is "none".
    ...
    case "GET /api/reporting/status": {
      ...
      // The caller's own tier rides along so the UI can offer the report
      // generator only to "super" without a second permission lookup.
      const permission = typeof store.getAiUserPermission === "function"
        ? await store.getAiUserPermission(me.username)
        : null;
      const integration = typeof store.getIntegration === "function"
      ...
      return send(res, 200, {
        tier: reportingAccess(me, permission),
    ...
    // The owner-level report writer: reporting.js metrics + Task Hub work
    // context composed into a downloadable .docx (lib/report-writer.js). The
    // "super" reporting tier only — every dashboard route above stays on the
    // advanced gate. The writer falls back to a deterministic report when the
    // AI provider is unconfigured, so this route must not gate on aiPolicy.
    case "POST /api/reporting/report": {
      const me = await reportGeneratorUser(req, res, store);
      if (!me) return;
      const body = await readBody(req);
      const to = clean(body.to, 10) || todayStr();
      const from = clean(body.from, 10) || shiftDays(to, -29);
      if (!validDay(from) || !validDay(to)) {
        return send(res, 400, { error: "Dates must be YYYY-MM-DD." });
      }
      if (from > to) return send(res, 400, { error: "Range start must not be after range end." });
      if (dayCount(from, to) > 365) {
        return send(res, 400, { error: "Report range is limited to 366 days." });
      }
      const audience = oneOf(clean(body.audience, 10) || "internal", ["internal", "client"], null);
      if (!audience) return send(res, 400, { error: 'Audience must be "internal" or "client".' });
      const writer = reportWriterModule();
      if (!writer || typeof writer.writeReport !== "function" || typeof writer.buildDocx !== "function") {
        return send(res, 503, { error: "The report writer is not available on this deployment yet." });
      }
      const label = `${audience} report ${from} → ${to}`;
      try {
        const report = await writer.writeReport(store, { from, to, audience });
        const docx = await writer.buildDocx({ title: report.title, sections: report.sections });
        await store.aiLog({
          username: me.username, kind: "report", question: label, status: "ok",
          answer: typeof report.title === "string" && report.title ? report.title : label,
        });
        return send(res, 200, {
          fileName: `neonmonki-${audience}-report-${from}-${to}.docx`,
          title: typeof report.title === "string" && report.title ? report.title : `NEONMONKI ${audience} report`,
          html: typeof report.html === "string" ? report.html : "",
          docxBase64: Buffer.from(docx).toString("base64"),
        });
      } catch (error) {
        console.error("[task-hub] report generation failed:", error);
        try {
          await store.aiLog({
            username: me.username, kind: "report", question: label,
            status: "error", error: (error && error.code) || "report",
          });
        } catch { /* the audit write must never mask the original failure */ }
        return send(res, 500, { error: "The report could not be generated. Please try again." });
      }
    }

    /* ------------------------------ report library ------------------------------ */
    // Curated report links (Drive docs, decks), scoped per client: team +
    // super admin read all and add, super admin edits and deletes, client
    // accounts read only their own client's rows, external partners get none.
    // Rows live in report_library (migration 011), exposed through
    // store.reportsList/reportInsert/reportUpdate/reportDelete.

    case "GET /api/reports": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role === "external") return send(res, 403, { error: "Reports are not enabled for this account." });
      if (typeof store.reportsList !== "function") {
        return send(res, 503, { error: "The report library is not available on this deployment yet." });
      }
      const reports = await store.reportsList();
      if (me.role === "client") {
        const clientId = me.clientId || "neonmonki";
        return send(res, 200, { reports: reports.filter((r) => (r.clientId || "neonmonki") === clientId) });
      }
    ...
    case "POST /api/reports": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (!isTeamRole(me.role)) return send(res, 403, { error: "Only the team can add reports." });
      if (typeof store.reportInsert !== "function") {
        return send(res, 503, { error: "The report library is not available on this deployment yet." });
      }
      const body = await readBody(req);
      const parsed = reportFieldsFrom(body);
      if (parsed.error) return send(res, 400, { error: parsed.error });
      let clientId = "neonmonki";
      if (body.clientId !== undefined && body.clientId !== null && clean(body.clientId, 60)) {
        const checked = await clientIdChecked(store, body.clientId);
        if (checked.error) return send(res, 400, { error: checked.error });
        clientId = checked.clientId;
      }
      const report = await store.reportInsert({ ...parsed.fields, clientId, createdBy: me.username });
      await store.logActivity({
        ts: new Date().toISOString(), taskId: null, by: me.name,
        text: `added report “${report.title}” (${report.periodMonth})`,
      });
      return send(res, 201, { report });
    }

    case "PATCH /api/reports/:id": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.reportUpdate !== "function") {
        return send(res, 503, { error: "The report library is not available on this deployment yet." });
      }
      const body = await readBody(req);
      const parsed = reportFieldsFrom(body, { partial: true });
      if (parsed.error) return send(res, 400, { error: parsed.error });
      if (!Object.keys(parsed.fields).length) {
        return send(res, 400, { error: "No report fields to update." });
      }
      const report = await store.reportUpdate(parts[0], parsed.fields);
    ...
    case "DELETE /api/reports/:id": {
      ...
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.reportDelete !== "function") {
        return send(res, 503, { error: "The report library is not available on this deployment yet." });
      }
      const existing = typeof store.reportsList === "function"
        ? (await store.reportsList()).find((r) => String(r.id) === String(parts[0]))
        : null;
      await store.reportDelete(parts[0]);
      await store.logActivity({
        ts: new Date().toISOString(), taskId: null, by: me.name,
        text: existing
          ? `deleted report “${existing.title}” (${existing.periodMonth})`
          : `deleted report ${parts[0]}`,
      });
      return send(res, 200, { ok: true });
    }
    ...
    case "GET /api/integrations/hyros/oauth/start": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
    ...
    case "POST /api/integrations/hyros/sync": {
      // Manual reporting refresh is a Super Admin control — never a client one.
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const body = await readBody(req);
    ...
    // route stays closed. The same run refreshes connected report platforms
    // (GSC trailing window + Clarity snapshot) after the Hyros sync.
    case "GET /api/cron/hyros-sync": {
      const secret = process.env.CRON_SECRET || "";
      const auth = String(req.headers.authorization || "");
      if (!secret || auth !== `Bearer ${secret}`) return send(res, 401, { error: "Unauthorized." });
      const out = await performHyrosSync(store, {});
      const platforms = platformsModule();
      const platformResults = {};
      if (platforms && typeof store.getIntegration === "function") {
        for (const platform of platforms.PLATFORM_IDS) {
          try {
            const integration = await store.getIntegration(platform);
            if (integration && integration.status === "connected") {
              platformResults[platform] = await platforms.syncPlatform(store, platform);
            }
          } catch (e) {
            platformResults[platform] = { ok: false, error: String((e && e.message) || "sync failed").slice(0, 200) };
          }
        }
      }
      // Discord: the same daily run delivers overdue-task pings (deduped per
      // task/person/due-date inside the module). Never fails the cron route.
      let discordOverdue = null;
      const discord = discordModule();
      if (discord) {
        try {
          discordOverdue = await discord.runOverdueSweep(store, { origin: requestOrigin(req) });
        } catch (e) {
          discordOverdue = { ok: false, error: String((e && e.message) || "sweep failed").slice(0, 200) };
        }
      }
      return send(res, out.status, { ...out.body, platforms: platformResults, discord: { overdue: discordOverdue } });
    }
    ...
    /* ------------------------------ platform reports ------------------------------ */
    // Platform Reports is the owner-level analytics surface: the Smart
    // Reporting gate (advanced/super tier) applies to every read, and all
    // connect/sync/disconnect mutations are super-admin-only. Access can be
    // granted to others later from the Control Panel by raising their tier.

    case "GET /api/platforms": {
      const me = await smartReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const googleCfg = await platforms.googleOAuthConfig(store);
      const names = {
        gsc: "Google Search Console", clarity: "Microsoft Clarity",
        ga4: "Google Analytics 4", google_ads: "Google Ads",
        meta_ads: "Meta Ads", salesforce: "Salesforce",
      };
      const out = [];
      for (const id of ["gsc", "ga4", "google_ads", "meta_ads", "clarity", "salesforce"]) {
        out.push({ id, name: names[id], ...(await platforms.platformStatus(store, id)) });
      }
      return send(res, 200, {
        platforms: out,
        googleConfigured: !!googleCfg,
        googleConfigSource: googleCfg ? googleCfg.source : "",
      });
    }

    case "GET /api/platforms/:platform/report": {
      const me = await smartReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      ...
      const range = parseReportingRange(url);
      if (range.error) return send(res, 400, { error: range.error });
      if (platform === "gsc") return send(res, 200, await platforms.gscReport(store, range));
      if (platform === "clarity") return send(res, 200, await platforms.clarityReport(store));
      const spec = platforms.platformReportSpec(platform);
      if (!spec) return send(res, 404, { error: "Unknown platform." });
      return send(res, 200, await platforms.metricReport(store, platform, range, spec));
    }

    case "POST /api/platforms/google/config": {
      // The Google OAuth client (id + secret) is stored encrypted in the
      // database — managed from the UI, no Vercel env required. Env vars
      // remain a fallback when no stored config exists.
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const body = await readBody(req);
      const clientId = clean(body.clientId, 200);
      const clientSecret = String(body.clientSecret || "").trim();
      if (!/^[\w.-]+\.apps\.googleusercontent\.com$/.test(clientId)) {
        return send(res, 400, { error: "The client ID should end with .apps.googleusercontent.com." });
      }
      if (clientSecret.length < 12 || clientSecret.length > 200 || /\s/.test(clientSecret)) {
        return send(res, 400, { error: "Paste the full client secret (no spaces)." });
      }
      await store.putIntegration("google_oauth", {
        name: "Google OAuth client",
        status: "connected",
        accountName: clientId,
        apiKeyEncrypted: ai.encryptSecret(clientSecret, "neonmonki-task-hub:platform-reports:v1"),
        meta: { clientId },
        updatedAt: new Date().toISOString(),
      });
      await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: "saved the Google OAuth client for Platform Reports" });
      return send(res, 200, { ok: true });
    }

    case "GET /api/platforms/combined": {
      const me = await smartReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms || typeof platforms.combinedReport !== "function") {
        return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      }
      const url = new URL(req.url, "http://localhost");
      const range = parseReportingRange(url);
      if (range.error) return send(res, 400, { error: range.error });
      return send(res, 200, await platforms.combinedReport(store, range));
    }

    case "GET /api/platforms/clarity/explore": {      const me = await smartReportingUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const url = new URL(req.url, "http://localhost");
      const range = parseReportingRange(url);
      if (range.error) return send(res, 400, { error: range.error });
      return send(res, 200, await platforms.clarityExplore(store, {
        ...range,
        dim: clean(url.searchParams.get("dim"), 20),
        metric: clean(url.searchParams.get("metric"), 120),
      }));
    }

    case "GET /api/platforms/gsc/oauth/start": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      const fail = (reason) => {
        res.writeHead(302, { Location: `/?platform=${encodeURIComponent(reason)}#/platformreports` });
        return res.end();
      };
      if (!platforms || !(await platforms.googleOAuthConfig(store))) return fail("gsc-not-configured");
      // One Google consent can connect several services at once (GSC + GA4 +
      // Google Ads) — the ?services= list decides which scopes are requested.
      const url = new URL(req.url, "http://localhost");
      const services = [...new Set(clean(url.searchParams.get("services"), 60).split(",").map((s) => s.trim()).filter(Boolean)
        .filter((s) => ["gsc", "ga4", "google_ads"].includes(s)))];
      const wanted = services.length ? services : ["gsc"];
      const scopeMap = {
        gsc: [], ga4: [platforms.GA4_SCOPE], google_ads: [platforms.GOOGLE_ADS_SCOPE],
      };
      const extraScopes = wanted.flatMap((s) => scopeMap[s]);
      const state = crypto.randomBytes(24).toString("base64url");
      const url2 = await platforms.gscAuthUrl(store, requestOrigin(req), state, extraScopes);
      await store.putIntegration("google_oauth", {
        oauthPending: { stateHash: platforms.sha256Text(state), services: wanted, createdAt: new Date().toISOString() },
        updatedAt: new Date().toISOString(),
      });
      res.writeHead(302, { Location: url2, "Cache-Control": "private, no-store" });
      return res.end();
    }

    case "GET /api/platforms/gsc/oauth/callback": {
      // Google returns here after sign-in; the SameSite=Lax session cookie
      // rides along, so the super-admin gate still applies. One consent can
      // connect several Google services (GSC, GA4, Google Ads) at once.
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      const fail = (reason) => {
      ...
      if (!platforms) return fail("gsc-unavailable");
      try {
        const url = new URL(req.url, "http://localhost");
        const q = url.searchParams;
        if (q.get("error")) return fail("gsc-denied");
        const code = clean(q.get("code"), 500);
        const state = clean(q.get("state"), 200);
        if (!code || !state) return fail("gsc-invalid");
        const secrets = await store.getIntegrationSecret("google_oauth");
        const pending = (secrets && secrets.oauthPending) || {};
        const fresh = pending.createdAt && (Date.now() - Date.parse(pending.createdAt)) < 10 * 60 * 1000;
        if (!fresh || !pending.stateHash || platforms.sha256Text(state) !== pending.stateHash) {
          return fail("gsc-state");
        }
        const services = Array.isArray(pending.services) && pending.services.length ? pending.services : ["gsc"];
        const tokens = await platforms.gscExchangeCode(store, requestOrigin(req), code);
        if (!tokens.refreshToken) {
          // Google only hands a refresh token on a fresh consent. Ask the admin
          // to redo the flow rather than storing a session that dies in an hour.
          return fail("gsc-no-refresh");
        }
        const email = await platforms.gscUserEmail(tokens.accessToken);
        const now = new Date().toISOString();
        const tokenFields = {
          authMethod: "oauth",
          oauthAccessTokenEncrypted: ai.encryptSecret(tokens.accessToken, "neonmonki-task-hub:platform-reports:v1"),
          oauthAccessExpiresAt: tokens.expiresAt,
          oauthRefreshTokenEncrypted: ai.encryptSecret(tokens.refreshToken, "neonmonki-task-hub:platform-reports:v1"),
          oauthPending: {},
        };
        const connected = [];

        if (services.includes("gsc")) {
          const sites = await platforms.gscListSites(tokens.accessToken);
          if (!sites.length) return fail("gsc-no-sites");
          const pick = sites.find((s) => /neonmonki/i.test(s.siteUrl)) || sites[0];
          await store.putIntegration("gsc", {
            name: "Google Search Console", status: "connected",
            accountName: email || pick.siteUrl,
            ...tokenFields,
            meta: { siteUrl: pick.siteUrl, permissionLevel: pick.permissionLevel || "" },
            lastError: "", connectedBy: me.username, connectedAt: now, updatedAt: now,
          });
          connected.push("GSC");
        }
        if (services.includes("ga4")) {
          const properties = await platforms.ga4ListProperties(tokens.accessToken);
          if (!properties.length) return fail("ga4-no-properties");
          const pick = properties.find((p) => /neonmonki/i.test(`${p.displayName} ${p.account}`)) || properties[0];
          await store.putIntegration("ga4", {
            name: "Google Analytics 4", status: "connected",
            accountName: email || pick.displayName,
            ...tokenFields,
            meta: { propertyId: pick.propertyId, propertyLabel: pick.displayName },
            lastError: "", connectedBy: me.username, connectedAt: now, updatedAt: now,
          });
          connected.push("GA4");
        }
        if (services.includes("google_ads")) {
          // The Ads card stores developer token + customer ID first (prepare);
          // this callback adds the OAuth tokens onto that row.
          const prepared = await store.getIntegration("google_ads");
          if (!prepared || !prepared.meta || !prepared.meta.customerId) return fail("googleads-not-prepared");
          await store.putIntegration("google_ads", {
            name: "Google Ads", status: "connected",
            accountName: email || prepared.meta.customerId,
            ...tokenFields,
            meta: prepared.meta,
            lastError: "", connectedBy: me.username, connectedAt: now, updatedAt: now,
          });
        ...
        await store.putIntegration("google_oauth", { oauthPending: {}, updatedAt: new Date().toISOString() });
        await store.logActivity({
          ts: now, taskId: null, by: me.name,
          text: `connected ${connected.join(" + ")} via Google sign-in`,
        });
        // Kick the first syncs; each connector reports its own failure without
        // blocking the others.
        for (const service of services) {
          const result = await platforms.syncPlatform(store, service);
          await store.logActivity({
            ts: new Date().toISOString(), taskId: null, by: me.name,
            text: result.ok
              ? `synced ${service} (${result.recordsIn} rows)`
              : `${service} connected; first sync pending (${result.error || "retry from Platform Reports"})`,
          });
        }
        res.writeHead(302, { Location: "/?platform=gsc-connected#/platformreports" });
        return res.end();
      } catch (e) {
        console.error("[google-oauth] callback failed:", e && e.status ? `HTTP ${e.status}` : "error");
        return fail("gsc-failed");
      }
    }

    case "POST /api/platforms/clarity/connect": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const body = await readBody(req);
      // Pasted tokens often carry stray whitespace/newlines — strip them all.
      const token = String(body.token || "").replace(/\s+/g, "");
      if (token.length < 12 || token.length > 2000) {
        return send(res, 400, { error: "The Clarity API token must be 12–2000 characters." });
      }
      // A non-ASCII character (smart quote, invisible paste artifact) makes the
      // Authorization header invalid — the request would never leave.
      if (!/^[\x21-\x7E]+$/.test(token)) {
        return send(res, 400, { error: "The token contains an invalid character — copy it again from Clarity → Settings → Data Export API." });
      }
      const test = await platforms.clarityValidate(token);
      if (!test.ok) {
        await store.logActivity({
          ts: new Date().toISOString(), taskId: null, by: me.name,
          text: `Clarity connect failed: ${test.error}`,
        });
        return send(res, 400, { error: test.error });
      }
      const now = new Date().toISOString();
      await store.putIntegration("clarity", {
        name: "Microsoft Clarity",
        status: "connected",
        authMethod: "token",
        accountName: "Clarity project",
        apiKeyEncrypted: ai.encryptSecret(token, "neonmonki-task-hub:platform-reports:v1"),
        meta: { propertyLabel: "Clarity project (token-scoped)" },
        lastError: "",
        connectedBy: me.username,
        connectedAt: now,
        updatedAt: now,
      });
      await store.logActivity({ ts: now, taskId: null, by: me.name, text: "connected Microsoft Clarity" });
      const result = await platforms.syncPlatform(store, "clarity");
      if (!result.ok) {
        await store.logActivity({
          ts: new Date().toISOString(), taskId: null, by: me.name,
          text: `Clarity first sync failed: ${result.error}`,
        });
      }
      return send(res, 200, { ok: !!result.ok, recordsIn: result.recordsIn || 0, error: result.ok ? "" : result.error || "" });
    }

    case "POST /api/platforms/google_ads/prepare": {
      // Google Ads needs a developer token + customer ID before the OAuth hop;
      // they are stored (token encrypted) and the account card then bounces to
      // the Google sign-in with the adwords scope.
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const body = await readBody(req);
      const developerToken = String(body.developerToken || "").replace(/\s+/g, "");
      const customerId = platforms.adsCustomerId(body.customerId);
      if (developerToken.length < 8 || developerToken.length > 200) {
        return send(res, 400, { error: "Paste the developer token from Google Ads → Admin → API Center." });
      }
      if (!customerId || customerId.length < 6 || customerId.length > 12) {
        return send(res, 400, { error: "Enter the Google Ads customer ID (digits only, dashes optional)." });
      }
      await store.putIntegration("google_ads", {
        name: "Google Ads",
        status: "disconnected",
        apiKeyEncrypted: ai.encryptSecret(developerToken, "neonmonki-task-hub:platform-reports:v1"),
        meta: { customerId },
        updatedAt: new Date().toISOString(),
      });
      return send(res, 200, { ok: true });
    ...
    case "POST /api/platforms/meta_ads/connect": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const body = await readBody(req);
      const token = String(body.token || "").replace(/\s+/g, "");
      const adAccountId = platforms.metaAdAccount(body.adAccountId);
      if (token.length < 12 || token.length > 2000) {
        return send(res, 400, { error: "Paste the Meta Marketing API access token." });
      }
      if (!/^\d{5,20}$/.test(adAccountId)) {
        return send(res, 400, { error: "Enter the ad account ID (the digits after act_)." });
      }
      const test = await platforms.metaValidate(token, adAccountId);
      if (!test.ok) {
        await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: `Meta Ads connect failed: ${test.error}` });
        return send(res, 400, { error: test.error });
      }
      const now = new Date().toISOString();
      await store.putIntegration("meta_ads", {
        name: "Meta Ads",
        status: "connected",
        authMethod: "token",
      ...
      await store.logActivity({ ts: now, taskId: null, by: me.name, text: `connected Meta Ads (act_${adAccountId})` });
      const result = await platforms.syncPlatform(store, "meta_ads");
      return send(res, 200, { ok: !!result.ok, recordsIn: result.recordsIn || 0, error: result.ok ? "" : result.error || "" });
    }

    case "POST /api/platforms/salesforce/connect": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const body = await readBody(req);
      let instanceUrl = String(body.instanceUrl || "").trim().replace(/\/$/, "");
      const sfOverride = (process.env.SALESFORCE_BASE_URL || "").replace(/\/$/, "");
      if (sfOverride && instanceUrl === sfOverride) {
        // test hook: the stub base URL stands in for a *.salesforce.com host
      } else if (!/^https:\/\/[\w.-]+\.salesforce\.com$/.test(instanceUrl) && !/^https:\/\/[\w.-]+\.my\.salesforce\.com$/.test(instanceUrl)) {
        return send(res, 400, { error: "Instance URL looks like https://yourorg.my.salesforce.com" });
      }
      const clientId = String(body.clientId || "").trim();
      const clientSecret = String(body.clientSecret || "").replace(/\s+/g, "");
      if (!clientId || !clientSecret) return send(res, 400, { error: "Both the consumer key and consumer secret are required." });
      try {
        await platforms.salesforceToken(instanceUrl, clientId, clientSecret);
      } catch (e) {
        const msg = `Salesforce connect failed (HTTP ${(e && e.status) || "?"}). Check the connected app credentials.`;
        await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: msg });
        return send(res, 400, { error: msg });
      }
      const now = new Date().toISOString();
      await store.putIntegration("salesforce", {
        name: "Salesforce",
        status: "connected",
        authMethod: "client_credentials",
        accountName: instanceUrl.replace(/^https:\/\//, ""),
        apiKeyEncrypted: ai.encryptSecret(clientSecret, "neonmonki-task-hub:platform-reports:v1"),
        meta: { instanceUrl, clientId },
        lastError: "",
        connectedBy: me.username,
        connectedAt: now,
        updatedAt: now,
      });
      await store.logActivity({ ts: now, taskId: null, by: me.name, text: `connected Salesforce (${instanceUrl})` });
      const result = await platforms.syncPlatform(store, "salesforce");
      return send(res, 200, { ok: !!result.ok, recordsIn: result.recordsIn || 0, error: result.ok ? "" : result.error || "" });
    }

    case "POST /api/platforms/:platform/sync": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const platform = parts[0];
      if (!platforms.PLATFORM_IDS.includes(platform)) return send(res, 404, { error: "Unknown platform." });
      const result = await platforms.syncPlatform(store, platform);
      return send(res, result.ok ? 200 : 502, result);
    }

    case "POST /api/platforms/:platform/disconnect": {
      const me = await superAdminUser(req, res, store);
      if (!me) return;
      const platforms = platformsModule();
      if (!platforms) return send(res, 503, { error: "Platform reports are not available on this deployment yet." });
      const platform = parts[0];
      if (!platforms.PLATFORM_IDS.includes(platform)) return send(res, 404, { error: "Unknown platform." });
      const deleted = typeof store.platformDailyDelete === "function"
        ? await store.platformDailyDelete({ platform })
        : 0;
      const now = new Date().toISOString();
      await store.putIntegration(platform, {
        status: "disconnected", accountName: "",
        apiKeyEncrypted: "",
        oauthAccessTokenEncrypted: "", oauthRefreshTokenEncrypted: "",
        oauthAccessExpiresAt: null, oauthPending: {},
        meta: {}, lastSyncAt: null, lastError: "",
        disconnectedBy: me.username, disconnectedAt: now, updatedAt: now,
      });
      await store.logActivity({
        ts: now, taskId: null, by: me.name,
        text: `disconnected ${{ gsc: "Google Search Console", clarity: "Microsoft Clarity", ga4: "Google Analytics 4", google_ads: "Google Ads", meta_ads: "Meta Ads", salesforce: "Salesforce" }[platform] || platform} (${deleted} rows removed)`,
      });
    ...
    /* ------------------------------ clients ------------------------------ */
    // This is a single-client workspace: NEONMONKI. There is deliberately no
    // client registry API — client accounts (Adika, Andy, Dustin, …) are
    // NEONMONKI people managed from Control Panel → Users, all scoped to the
    // built-in "neonmonki" client id (see clientIdChecked).
    ...
    case "POST /api/admin/users": {
      ...
      const discordUserId = clean(body.discordUserId, 25);
      if (discordUserId && !/^\d{5,25}$/.test(discordUserId)) {
        return send(res, 400, { error: "Discord User ID is the numeric ID from Discord (User Settings → Advanced → Developer Mode, then right-click the user → Copy User ID)." });
      }
      // clientId is meaningful for client accounts only; every other role
      // stores "" so scoping never keys off a team/external user.
      let clientId = "";
      if (role === "client") {
        const checked = await clientIdChecked(store, body.clientId);
        if (checked.error) return send(res, 400, { error: checked.error });
      ...
      // External partners start with AI off and reporting disabled — the super
      // admin grants either explicitly from the Control Panel.
      if (role === "external" && typeof store.putAiUserPermission === "function") {
        await store.putAiUserPermission(username, { enabled: false, reporting: "none", updatedBy: me.username });
      }
    ...
    case "PATCH /api/admin/users/:username": {
      ...
      if ("discordUserId" in body) {
        ...
        fields.discordUserId = discordUserId;
      }
      ...
      // clientId follows the effective role: validated for client accounts
      // (default "neonmonki" when a new client has none), always "" otherwise.
      if ("clientId" in body || "role" in body) {
        if ((fields.role || target.role) === "client") {
          if ("clientId" in body) {
            const checked = await clientIdChecked(store, body.clientId);
            if (checked.error) return send(res, 400, { error: checked.error });
            fields.clientId = checked.clientId;
          } else if (!target.clientId) {
            fields.clientId = "neonmonki";
          }
        } else {
          fields.clientId = "";
        }
    ...
    case "PATCH /api/admin/departments/:deptId": {
      ...
      const department = normalizeDepartment({
        ...
        external: "external" in body ? !!body.external : current.external === true,
    ...
    /* --------------------------- Discord integration --------------------------- */
    // Discord is the real-time notification layer. Config (bot token, guild,
    // default channel, per-type toggles) lives in integration_connections under
    // the id "discord" — the token is write-only/encrypted like every other
    // connector credential. Boards are the client registry: each board routes
    // to its own Discord channel. All of this is super-admin only.

    case "GET /api/admin/discord": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      const discord = discordModule();
      if (!discord) return send(res, 503, { error: "Discord layer unavailable on this deployment." });
      const cfg = await discord.getConfig(store);
      const boards = await discord.boardsWithDiscord(store);
      const recent = typeof store.discordLogRecent === "function"
        ? await store.discordLogRecent(20).catch(() => [])
        : [];
      return send(res, 200, { config: discord.publicConfig(cfg), boards, recent });
    }

    case "PUT /api/admin/discord": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      const discord = discordModule();
      if (!discord) return send(res, 503, { error: "Discord layer unavailable on this deployment." });
      const body = await readBody(req);
      const guildId = clean(body.guildId, 30);
      const defaultChannelId = clean(body.defaultChannelId, 30);
      const snowflake = /^\d{5,25}$/;
      if (guildId && !snowflake.test(guildId)) {
        return send(res, 400, { error: "Server ID must be the numeric Discord guild ID (right-click your server → Copy Server ID with Developer Mode on)." });
      }
      if (defaultChannelId && !snowflake.test(defaultChannelId)) {
        return send(res, 400, { error: "Default channel ID must be the numeric Discord channel ID (right-click the channel → Copy Channel ID)." });
      }
      const botToken = String(body.botToken || "").trim();
      if (botToken && (botToken.length < 20 || /\s/.test(botToken))) {
        return send(res, 400, { error: "That doesn't look like a Discord bot token — paste the token from Developer Portal → Bot → Reset Token." });
      }
      let notifications;
      if (body.notifications && typeof body.notifications === "object") {
        notifications = {};
        for (const type of discord.NOTIFICATION_TYPES) {
          notifications[type] = body.notifications[type] !== false;
        }
      }
      const cfg = await discord.saveSettings(store, {
      ...
      return send(res, 200, { config: discord.publicConfig(cfg) });
    }

    case "POST /api/admin/discord/test": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      const discord = discordModule();
      ...
      const channelId = clean(body && body.channelId, 30);
      if (channelId && !/^\d{5,25}$/.test(channelId)) {
        return send(res, 400, { error: "Channel ID must be the numeric Discord channel ID." });
      }
      const result = await discord.testConnection(store, { channelId });
      return send(res, result.ok ? 200 : 400, result);
    }

    /* Boards: the client registry doubles as the board list — each board gets
     * its own Discord channel. Adding a board here needs no code change; new
     * tasks can be scoped to it immediately (tasks already carry client_id). */

    case "POST /api/admin/boards": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.clientInsert !== "function") return send(res, 503, { error: "Board registry unavailable." });
      const body = await readBody(req);
      const name = clean(body.name, 60);
      if (!name) return send(res, 400, { error: "Board name is required." });
      const id = slugify(body.id || name);
      if (!id) return send(res, 400, { error: "Board name must contain at least one letter or number." });
      const discordChannelId = clean(body.discordChannelId, 30);
      if (discordChannelId && !/^\d{5,25}$/.test(discordChannelId)) {
        return send(res, 400, { error: "Discord channel ID must be numeric (right-click the channel → Copy Channel ID)." });
      }
      const existing = await clientsRegistry(store);
      if (existing.some((c) => c.id === id)) return send(res, 409, { error: "A board with this id already exists." });
      const board = await store.clientInsert({
        id, name, active: true, notes: clean(body.notes, 300),
        discordChannelId, discordEnabled: body.discordEnabled !== false,
      });
      await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: `added board ${name} (${id})` });
      return send(res, 201, { board });
    }

    case "PATCH /api/admin/boards/:id": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      if (typeof store.clientUpdate !== "function") return send(res, 503, { error: "Board registry unavailable." });
      const id = parts[0];
      const existing = await clientsRegistry(store);
      if (!existing.some((c) => c.id === id)) return send(res, 404, { error: "Board not found." });
      const body = await readBody(req);
      const fields = {};
      if ("name" in body) {
        const name = clean(body.name, 60);
      ...
      if ("active" in body) fields.active = !!body.active;
      if ("notes" in body) fields.notes = clean(body.notes, 300);
      if ("discordChannelId" in body) {
        const channelId = clean(body.discordChannelId, 30);
        if (channelId && !/^\d{5,25}$/.test(channelId)) {
          return send(res, 400, { error: "Discord channel ID must be numeric (right-click the channel → Copy Channel ID)." });
        }
        fields.discordChannelId = channelId;
      }
      if ("discordEnabled" in body) fields.discordEnabled = body.discordEnabled !== false;
      if (!Object.keys(fields).length) return send(res, 400, { error: "Nothing to update." });
      const board = await store.clientUpdate(id, fields);
      await store.logActivity({ ts: new Date().toISOString(), taskId: null, by: me.name, text: `updated board ${(board && board.name) || id} (${Object.keys(fields).join(", ")})` });
    ...
    case "POST /api/ai/ask": {
      ...
      if (!access.allowed) {
        // Blocked asks are part of the AI history too — the super admin sees
        // who tried, when, and what the user was shown.
        try {
          await store.aiLog({
            username: me.username, kind: "ask", question,
            status: "blocked", answer: access.blocked.error, error: access.blocked.error,
          });
        } catch { /* audit must never block the response */ }
        return send(res, access.blocked.status, { error: access.blocked.error });
      }
      ...
      try {
        ...
        await store.aiLog({
          username: me.username, kind: "ask", question, answer: r.answer || "",
        ...
        await store.aiLog({
          ...
          answer: "Monki couldn't answer right now. Try again in a moment.",
    ...
    case "POST /api/ai/summarize/task/:id": {
      ...
      try {
        ...
        await store.aiLog({
          username: me.username, kind: "task_summary", question: task.id, answer: r.answer || "",
    ...
    case "POST /api/ai/summarize/channel/:cid": {
      ...
      try {
        ...
        await store.aiLog({
          username: me.username, kind: "channel_summary", question: channel.id, answer: r.answer || "",
          citations: r.citations, model: r.model,
          promptTokens: r.usage.prompt_tokens, completionTokens: r.usage.completion_tokens,
          latencyMs: r.latencyMs, status: "ok",
        });
    ...
    case "POST /api/ai/brief": {
      ...
      try {
        ...
        await store.aiLog({
          username: me.username, kind: "brief", answer: r.answer || "", model: r.model,
          promptTokens: r.usage.prompt_tokens, completionTokens: r.usage.completion_tokens,
          latencyMs: r.latencyMs, status: "ok",
        });
    ...
    case "POST /api/ai/report": {
      ...
      try {
        ...
        await store.aiLog({
          ...
          question: `${period} report ${from}..${to} (${audience})`, answer: r.answer || "",
          citations: r.citations, model: r.model,
          promptTokens: r.usage.prompt_tokens, completionTokens: r.usage.completion_tokens,
          latencyMs: r.latencyMs, status: "ok",
        });
     ...
     * answers, browsable by day. Days follow the workspace clock:
     * Asia/Karachi (UTC+5, no daylight saving). Super admin only. */
    case "GET /api/ai/admin/history": {
      const me = await getAuth(req, store);
      if (!me) return send(res, 401, { error: "Not signed in." });
      if (me.role !== "super_admin") return send(res, 403, { error: "Super admin only." });
      const url = new URL(req.url, "http://localhost");
      const dateRaw = clean(url.searchParams.get("date"), 10);
      if (dateRaw && !validDay(dateRaw)) return send(res, 400, { error: "Date must be YYYY-MM-DD." });
      const username = clean(url.searchParams.get("username"), 30).toLowerCase();
      const pktNow = new Date(Date.now() + 5 * 3600 * 1000).toISOString().slice(0, 10);
      const day = dateRaw || pktNow;
      const fromMs = Date.parse(`${day}T00:00:00+05:00`);
      const from = new Date(fromMs).toISOString();
      const to = new Date(fromMs + 24 * 3600 * 1000).toISOString();
      const entries = typeof store.aiAuditQuery === "function"
        ? await store.aiAuditQuery({ from, to, username, limit: 500 })
        : (await store.aiAuditList(1000)).filter((e) => e.ts >= from && e.ts < to && (!username || e.username === username));
      const users = await store.listUsers();
      const nameOf = new Map(users.map((u) => [u.username, u.name]));
      return send(res, 200, {
        date: day,
        timezone: "Asia/Karachi",
        entries: entries.map((e) => ({
          id: e.id, ts: e.ts, username: e.username,
          name: nameOf.get(e.username) || e.username,
          kind: e.kind, question: e.question || "", answer: e.answer || "",
          status: e.status, error: e.error || "",
          tools: e.tools || [], model: e.model || "",
        })),
      });
    }
    ...
    case "PATCH /api/ai/admin/users/:username": {
      ...
      if ("reporting" in body) {
        ...
        // permissions.reportingAccess). The legacy tier name "full" is
        // accepted and stored under its new name, "advanced". Anything else
        // is rejected, never silently coerced.
        const raw = clean(body.reporting, 10);
        const reporting = oneOf(raw === "full" ? "advanced" : raw, ["", "none", "basic", "advanced", "super"], null);
        if (reporting === null) {
          return send(res, 400, { error: 'Reporting access must be "none", "basic", "advanced", "super" or empty.' });
        }
...
const ROUTE_PATTERNS = [
  ...
  { re: /^\/api\/reports\/(\d+)$/, key: "PATCH /api/reports/:id", methods: ["PATCH"] },
  { re: /^\/api\/reports\/(\d+)$/, key: "DELETE /api/reports/:id", methods: ["DELETE"] },
  ...
  { re: /^\/api\/platforms\/(gsc|clarity|ga4|google_ads|meta_ads|salesforce)\/sync$/, key: "POST /api/platforms/:platform/sync", methods: ["POST"] },
  { re: /^\/api\/platforms\/(gsc|clarity|ga4|google_ads|meta_ads|salesforce)\/disconnect$/, key: "POST /api/platforms/:platform/disconnect", methods: ["POST"] },
  { re: /^\/api\/platforms\/(gsc|clarity|ga4|google_ads|meta_ads|salesforce)\/report$/, key: "GET /api/platforms/:platform/report", methods: ["GET"] },
  ...
  { re: /^\/api\/admin\/boards\/([\w-]+)$/, key: "PATCH /api/admin/boards/:id", methods: ["PATCH"] },
