...
/** Decrypt the stored Hyros key server-side; nothing key-shaped ever leaves. */
function hyrosConfig(integration) {
...
function hyrosPublicStatus(integration, recordCount) {
  ...
  return {
    ...
    lastError: connected ? clean(integration.lastError, 240) : "",
...
/**
 * Bounded historical backfill. The connector batches by week and returns a
 * `nextCursor` while more history remains; the cursor is persisted on the
 * integration record so a later invocation (Vercel function budget) continues
 * exactly where this one stopped. Everything stays idempotent in the store.
 */
async function runHyrosBackfill(store, hyros, cfg, days) {
  const integration = typeof store.getIntegration === "function"
    ...
    : null;
  let cursor = (integration && integration.backfill && integration.backfill.cursor) || null;
  ...
  while (!done && Date.now() < deadline) {
    const batch = await hyros.syncBackfill(cfg, { days, cursor, deadlineMs: Math.max(0, deadline - Date.now()) });
    if (!batch || !batch.ok) { error = sanitizeHyrosError(batch && batch.error, cfg.apiKey); break; }
    recordsIn += Number(batch.recordsIn) || 0;
    cursor = batch.nextCursor || null;
    done = batch.done === true || !cursor;
    await store.putIntegration(HYROS_INTEGRATION_ID, {
      backfill: { cursor, done }, updatedAt: new Date().toISOString(),
    });
  }
  const now = new Date().toISOString();
  if (run && run.id != null) {
    await store.syncRunUpdate(run.id, {
      status: error ? "error" : done ? "ok" : "partial",
  ...
  await store.putIntegration(HYROS_INTEGRATION_ID, {
    ...
    lastError: error,
    updatedAt: now,
    ...(done ? { backfill: { cursor: null, done: true } } : {}),
  });
  return { recordsIn, done, error };
}
...
async function performHyrosSync(store, { days = null } = {}) {
  ...
  const cfg = hyrosConfig(await store.getIntegrationSecret(HYROS_INTEGRATION_ID));
  if (!cfg.apiKey) return { status: 500, body: { error: "The stored Hyros key could not be read — reconnect the integration." } };
  ...
  const result = await hyros.syncIncremental(cfg, { from, to });
  const ok = !!(result && result.ok);
  const error = ok ? "" : sanitizeHyrosError(result && result.error, cfg.apiKey);
  const now = new Date().toISOString();
  if (run && run.id != null) {
    await store.syncRunUpdate(run.id, {
      status: ok ? "ok" : "error", recordsIn: ok ? Number(result.recordsIn) || 0 : 0,
...
async function route(req, res, key, parts) {
  ...
  switch (key) {
    ...
    case "GET /api/integrations/hyros/status": {
      ...
      return send(res, 200, hyrosPublicStatus(integration, recordCount));
    }
    ...
    case "POST /api/integrations/hyros/connect": {
      ...
      await store.putIntegration(HYROS_INTEGRATION_ID, {
        ...
        historicalDays,
        lastError: "",
        backfill: { cursor: null, done: false },
    ...
    case "POST /api/integrations/hyros/test": {
      ...
      const cfg = hyrosConfig(await store.getIntegrationSecret(HYROS_INTEGRATION_ID));
      if (!cfg.apiKey) return send(res, 500, { error: "The stored Hyros key could not be read — reconnect the integration." });
